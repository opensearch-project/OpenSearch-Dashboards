...
export const PanelGroupOptionsAndContextActions: React.FC = () => {
  ...
  const actions = [
    ...
    sampleAction('test-11', 10, 'This group', 'dashboardApp', customActionGrouping),
    sampleAction('test-12', 9, 'has been', 'dashboardApp', customActionGrouping),
    sampleAction('test-13', 8, 'flattened using', 'cloudStormy', customActionGrouping),
    sampleAction('test-14', 7, 'isFlattened property', 'link', customActionGrouping),
  ];
    ...
    <EuiPopover
      button={
        <EuiButton onClick={() => setOpen((x) => !x)}>
          Custom actions using a flattened group
        </EuiButton>
      }
      isOpen={open}
