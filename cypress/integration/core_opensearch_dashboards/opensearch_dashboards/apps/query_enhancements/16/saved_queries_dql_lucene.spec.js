...
/**
 * Generate test configurations for DQL and Lucene only
 * Filter from all configurations to get only DQL and Lucene tests
 */
const generateDQLLuceneTestConfigurations = () => {
  const allTestConfigurations = generateAllTestConfigurations(generateSavedTestConfiguration);

  // Filter to only include DQL and Lucene languages
  return allTestConfigurations.filter(
    (config) =>
      config.language === QueryLanguages.DQL.name || config.language === QueryLanguages.Lucene.name
  );
};

const runSavedQueriesUITests = () => {
  describe('saved queries UI - DQL and Lucene', () => {
    before(() => {
    ...
    const testConfigurations = generateDQLLuceneTestConfigurations();
...
prepareTestSuite('Saved Queries - DQL and Lucene', runSavedQueriesUITests);
