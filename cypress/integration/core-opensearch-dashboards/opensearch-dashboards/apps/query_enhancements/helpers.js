...
export function getDocTableRow(rowNumber) {
  ...
  export function getDocTableField(columnNumber, rowNumber) {
    return getDocTableRow(rowNumber)
      .findElementByTestId('docTableField')
      .eq(columnNumber);
  }
  ...
  export function getExpandedDocRow(docTableRowNumber, expandedDocumentRowNumber) {
    return findDocTableExpandedDocRowsIn(
      getDocTableRow(docTableRowNumber + 1)
  ...
  export function selectIndexDatasetLanguage(datasetLanguage) {
    ...
    cy.getElementByTestId('advancedSelectorTimeFieldSelect').select("I don't want to use the time filter");
  ...
  export function toggleDocTableRow(rowNumber) {
    getDocTableRow(rowNumber).within(() => {
      cy
        .getElementByTestId('docTableExpandToggleColumn')
        .find('button').click();
    });
  }
  ...
  export function verifyDocTableFilterAction(
    ...
    getDocTableField(0, rowNumber).then(($field) => {
      ...
      cy.getElementByTestId('discoverQueryHits').should('have.text', expectedQueryHitsAfterFilterApplied); // checkQueryHitText must be in front of checking first line text to give time for DocTable to update.
      getDocTableField(0, rowNumber)
        .find('span span')
   ...
   */
  export function verifyDocTableFirstExpandedFieldFirstRowFilterForFilterOutExistsFilterButtons(isEnabled) {
  ...
  export function verifyDocTableFirstExpandedFieldFirstRowExistsFilterButtonFiltersCorrectField(
    ...
    getExpandedDocRowFieldName(docTableRowNumber, expandedDocumentRowNumber).then(
      ($expandedDocumentRowField) => {
        ...
        cy.getElementByTestId('discoverQueryHits').should('have.text', expectedQueryHitsAfterFilterApplied);
      }
    );
    cy.getElementByTestId('globalFilterBar').find('[aria-label="Delete"]').click();
    cy.getElementByTestId('discoverQueryHits').should('have.text', expectedQueryHitsWithoutFilter);
