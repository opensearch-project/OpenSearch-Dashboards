...
export function OsdAccessibleClickProvider() {
  return {
    restrict: 'A',
    controller: ($element) => {
    ...
    link: (scope, element, attrs) => {
      ...
      if (elementType === 'BUTTON') {
        throw new Error(`osdAccessibleClick doesn't need to be used on a button.`);
      }

      if (elementType === 'A' && attrs.href !== undefined) {
        throw new Error(
          `osdAccessibleClick doesn't need to be used on a link if it has a href attribute.`
        );
      }

      // We're emulating a click action, so we should already have a regular click handler defined.
      if (!attrs.ngClick) {
        throw new Error('osdAccessibleClick requires ng-click to be defined on its element.');
      }
