    ...
    return (
      response.map((remoteClusterConnection: { connectionAlias: any }) => {
        return {
          ...
          append: (
            <EuiText size="s">
              {i18n.translate('datasetManagement.createDatasetWizard.dataSource.description', {
  ...
  return datasources.map((ds) => {
    ...
    return {
      ...
      append: !!relatedCrossClusterConnections.length ? (
        <EuiBadge>
          {i18n.translate(
            'datasetManagement.createDatasetWizard.form.selectDataSource.crossClusterOptionBadge',
            {
