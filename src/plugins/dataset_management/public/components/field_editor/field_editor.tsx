  ...
  return [
    {
      id: '',
      defaultFieldFormat,
      title: i18n.translate('datasetManagement.defaultFormatDropDown', {
...
export class FieldEditor extends PureComponent<FieldEdiorProps, FieldEditorState> {
  ...
  constructor(props: FieldEdiorProps, context: DatasetManagmentContextValue) {
    super(props, context);

    const { spec, dataset } = props;
    ...
    this.state = {
      ...
      fieldTypes: [],
      fieldTypeFormats: [],
      existingFieldNames: dataset.fields.getAll().map((f: IFieldType) => f.name),
      ...
      isSaving: false,
      format: props.dataset.getFormatterForField(spec),
      spec: { ...spec },
    };
  ...
  async init(context: DatasetManagmentContextValue) {
    ...
    this.setState({
      isReady: true,
      isCreating: !dataset.fields.getByName(spec.name),
      ...
      fieldFormatId: get(dataset, ['fieldFormatMap', spec.name, 'type', 'id']),
  ...
  renderName() {
    ...
    return isCreating ? (
      <EuiCompressedFormRow
        label={i18n.translate('datasetManagement.nameLabel', { defaultMessage: 'Name' })}
        helpText={
          this.isDuplicateName() ? (
            <span>
              ...
              &nbsp;
              <FormattedMessage
                id="datasetManagement.mappingConflictLabel.mappingConflictDetail"
              ...
              the same name means you won't be able to query both fields at the same time."
                values={{
                  mappingConflict: (
                    <strong>
                      <FormattedMessage
                        id="datasetManagement.mappingConflictLabel.mappingConflictLabel"
        ...
        error={
          isInvalid
            ? i18n.translate('datasetManagement.nameErrorMessage', {
      ...
      >
        <EuiCompressedFieldText
          value={spec.name || ''}
          placeholder={i18n.translate('datasetManagement.namePlaceholder', {
  ...
  renderLanguage() {
    ...
    return spec.scripted ? (
      <EuiCompressedFormRow
        label={i18n.translate('datasetManagement.languageLabel', {
        ...
        helpText={
          isDeprecatedLang ? (
            <span>
              ...
              <strong>
                <FormattedMessage
                  id="datasetManagement.warningHeader"
                  defaultMessage="Deprecation Warning:"
                />
              </strong>
              &nbsp;
              <FormattedMessage
                id="datasetManagement.warningLabel.warningDetail"
              ...
              We recommend using {painlessLink} for new scripted fields."
                values={{
                  ...
                  painlessLink: (
                    ...
                    >
                      <FormattedMessage
                        id="datasetManagement.warningLabel.painlessLinkLabel"
  ...
  renderType() {
    ...
    return (
      <EuiCompressedFormRow
        label={i18n.translate('datasetManagement.typeLabel', { defaultMessage: 'Type' })}
      >
        <EuiCompressedSelect
          value={spec.type}
  ...
  renderTypeConflict() {
    ...
    const columns = [
      {
        field: 'type',
        name: i18n.translate('datasetManagement.typeLabel', { defaultMessage: 'Type' }),
        width: '100px',
      },
      {
        field: 'indices',
        name: i18n.translate('datasetManagement.indexNameLabel', {
    ...
    return (
      <div>
        ...
        <EuiCallOut
          ...
          title={
            <FormattedMessage
              id="datasetManagement.fieldTypeConflict"
        ...
        >
          <FormattedMessage
            id="datasetManagement.multiTypeLabelDesc"
  ...
  renderFormat() {
    ...
    const label = defaultFormat ? (
      ...
      <FormattedMessage id="datasetManagement.formatHeader" defaultMessage="Format" />
    );
    ...
    return (
      <Fragment>
        <EuiCompressedFormRow
          ...
          helpText={
            <FormattedMessage
              id="datasetManagement.formatLabel"
        ...
        {fieldFormatId ? (
          <FieldFormatEditor
            ...
            fieldFormatEditors={datasetManagementStart.fieldFormatEditors}
  ...
  renderPopularity() {
    ...
    return (
      <EuiCompressedFormRow
        label={i18n.translate('datasetManagement.popularityLabel', {
  ...
  renderScript() {
    ...
    const errorMsg = hasScriptError ? (
      <span data-test-subj="invalidScriptError">
        <FormattedMessage
          id="datasetManagement.scriptInvalidErrorMessage"
          defaultMessage="Script is invalid. View script preview for details"
        />
      </span>
    ) : (
      <FormattedMessage
        id="datasetManagement.scriptRequiredErrorMessage"
    ...
    return spec.scripted ? (
      <Fragment>
        <EuiCompressedFormRow
          fullWidth
          label={i18n.translate('datasetManagement.scriptLabel', { defaultMessage: 'Script' })}
          isInvalid={isInvalid}
        ...
        <EuiCompressedFormRow>
          <Fragment>
            <EuiText size="s">
              <FormattedMessage
                id="datasetManagement.script.accessWithLabel"
            ...
            <EuiText size="s">
              <EuiLink onClick={this.showScriptingHelp} data-test-subj="scriptedFieldsHelpLink">
                <FormattedMessage
                  id="datasetManagement.script.getHelpLabel"
  ...
  renderDeleteModal = () => {
    ...
    return this.state.showDeleteModal ? (
      <EuiConfirmModal
        ...
        cancelButtonText={i18n.translate('datasetManagement.deleteField.cancelButton', {
          defaultMessage: 'Cancel',
        })}
        confirmButtonText={i18n.translate('datasetManagement.deleteField.deleteButton', {
      ...
      >
        <p>
          <FormattedMessage
            id="datasetManagement.deleteFieldLabel"
        ...
        <EuiFlexGroup>
          <EuiFlexItem grow={false}>
            ...
            >
              {isCreating ? (
                <FormattedMessage
                  id="datasetManagement.actions.createButton"
                  defaultMessage="Create field"
                />
              ) : (
                <FormattedMessage
                  id="datasetManagement.actions.saveButton"
          ...
          <EuiFlexItem grow={false}>
            <EuiSmallButtonEmpty onClick={redirectAway} data-test-subj="fieldCancelButton">
              <FormattedMessage
                id="datasetManagement.actions.cancelButton"
                defaultMessage="Cancel"
              />
            </EuiSmallButtonEmpty>
          </EuiFlexItem>
              ...
              <EuiFlexGroup justifyContent="flexEnd">
                <EuiFlexItem grow={false}>
                  <EuiSmallButtonEmpty color="danger" onClick={this.showDeleteModal}>
                    <FormattedMessage
                      id="datasetManagement.actions.deleteButton"
                      defaultMessage="Delete"
                    />
                  </EuiSmallButtonEmpty>
                </EuiFlexItem>
  ...
  deleteField = () => {
    const { redirectAway, saveDataset } = this.props.services;
    const { dataset } = this.props;
    const { spec } = this.state;
  ...
  saveField = async () => {
    ...
    const { redirectAway, saveDataset } = this.props.services;
    const fieldExists = !!dataset.fields.getByName(field.name);

    let oldField: DatasetField['spec'];

    if (fieldExists) {
      oldField = dataset.fields.getByName(field.name)!.spec;
      dataset.fields.update(field);
    } else {
      dataset.fields.add(field);
    }

    if (!fieldFormatId) {
      dataset.fieldFormatMap[field.name] = undefined;
    } else {
      dataset.fieldFormatMap[field.name] = field.format;
    }
    ...
    return saveDataset(dataset)
      ...
      .catch(() => {
        if (oldField) {
          dataset.fields.update(oldField);
        } else {
      ...
      <div>
        <EuiText>
          <h3>
            {isCreating ? (
              <FormattedMessage
                id="datasetManagement.createHeader"
                defaultMessage="Create scripted field"
              />
            ) : (
              <FormattedMessage
                id="datasetManagement.editHeader"
