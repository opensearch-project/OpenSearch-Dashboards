...
describe('TraceAutoDetectCallout', () => {
  ...
  const renderWithContext = () => {
    return render(
      <I18nProvider>
        <OpenSearchDashboardsContextProvider services={mockServices as AgentTracesServices}>
  ...
  it('should respect localStorage dismissal', async () => {
    localStorageMock.setItem('agentTraces:traces:autoDetectDismissed', 'true');
  ...
  it('should clear dismissal if no trace datasets exist', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Trace Data Detected')).toBeInTheDocument();
      expect(localStorageMock.getItem('agentTraces:traces:autoDetectDismissed')).toBeNull();
    });
  });
  ...
  it('should dismiss callout when Dismiss button is clicked', async () => {
    ...
    await waitFor(() => {
      expect(localStorageMock.getItem('agentTraces:traces:autoDetectDismissed')).toBe('true');
      expect(screen.getByText('No Index Patterns')).toBeInTheDocument();
    });
  });
