...
describe('QueryPanel', () => {
  ...
  it('shows loading progress when query is loading', () => {
    ...
    renderWithProvider(<QueryPanel />);
    expect(screen.getByTestId('agentTracesQueryPanelIsLoading')).toBeInTheDocument();
  });
  ...
  it('does not show loading progress when not loading', () => {
    ...
    renderWithProvider(<QueryPanel />);
    expect(screen.queryByTestId('agentTracesQueryPanelIsLoading')).not.toBeInTheDocument();
  });
