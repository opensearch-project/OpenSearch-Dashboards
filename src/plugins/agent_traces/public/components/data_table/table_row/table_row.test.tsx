...
jest.mock('./table_row_content', () => ({
  TableRowContent: ({
    ...
    return (
      <tr className={row.isAnchor ? 'agentTracesDocTable__row--highlight' : ''}>
...
describe('TableRow', () => {
  ...
  it('toggles expansion when expand button is clicked', () => {
    ...
    expect(mockUseDynamicContext).toHaveBeenCalledWith({
      ...
      label: 'Row Entry',
      categories: ['agentTraces', 'chat', 'dynamic'],
    });
  });
  ...
  it('renders highlighted row when isAnchor is true', () => {
    ...
    const rowElement = document.querySelector('.agentTracesDocTable__row--highlight');
