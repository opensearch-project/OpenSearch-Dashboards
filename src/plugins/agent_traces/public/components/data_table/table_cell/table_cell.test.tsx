...
describe('TableCell', () => {
  ...
  it('renders as time field cell when isTimeField is true', () => {
    ...
    expect(cell).toHaveClass('agentTracesDocTableCell', 'eui-textNoWrap');
  ...
  it('renders as regular field cell when isTimeField is false', () => {
    ...
    const cell = screen.getByTestId('docTableField');
    expect(cell).toHaveClass('agentTracesDocTableCell', 'eui-textTruncate');
    expect(cell).not.toHaveClass('eui-textNoWrap');
  });

  it('renders content wrapper for all fields', () => {
    render(<TableCell {...defaultProps} isTimeField={false} />);
    expect(document.querySelector('.agentTracesDocTableCell__content')).toBeInTheDocument();
  });
    ...
    it('opens trace details URL when span ID link is clicked', () => {
      ...
      expect(window.open).toHaveBeenCalledWith(
        expect.stringContaining(
          "/app/agentTraces/traces/traceDetails#/?_a=(dataset:(id:'test-dataset-id',title:'test-dataset-title',type:'INDEX_PATTERN'),spanId:'test-span-id-123',traceId:'test-trace-id-456')"
        ),
        '_blank'
      );
    });
