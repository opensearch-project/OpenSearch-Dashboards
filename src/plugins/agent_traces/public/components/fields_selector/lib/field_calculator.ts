  ...
  if (NO_ANALYSIS_TYPES.includes(fieldType)) {
    return {
      error: i18n.translate(
        'agentTraces.discover.fieldChooser.fieldCalculator.analysisIsNotAvailableForGeoFieldsErrorMessage',
        {
  ...
  return values
    .filter((v) => {
      if (v instanceof Object && !Array.isArray(v)) {
        throw new Error(
          i18n.translate(
            'agentTraces.discover.fieldChooser.fieldCalculator.analysisIsNotAvailableForObjectFieldsErrorMessage',
            {
