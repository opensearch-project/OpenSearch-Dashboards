...
import { createHistogramConfigs, getDimensions } from './histogram_utils';
...
export const processTraceAggregationResults = ({
  ...
  const baseResults = requestAggResults
    ? {
        // For chart processing, skip expensive field counting and autocomplete updates
        // that defaultResultsProcessor performs - we only need the raw hits metadata
        hits: requestAggResults.hits,
        fieldCounts: {},
        dataset,
        elapsedMs: requestAggResults.elapsedMs,
      }
    : {
