...
import { resetAgentTracesStateActionCreator } from './reset_agent_traces_state';
import {
  setLegacyState,
  setQueryEditorState,
  setQueryState,
...
describe('resetAgentTracesStateActionCreator', () => {
  ...
  it('dispatches actions in correct order with preloaded state', async () => {
    const mockGetState = jest.fn().mockReturnValue({ query: { dataset: undefined } });
    await resetAgentTracesStateActionCreator(services, clearEditors)(mockDispatch, mockGetState);

    expect(getPreloadedState).toHaveBeenCalledWith(services, undefined);
