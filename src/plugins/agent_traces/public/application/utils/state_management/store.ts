    ...
    middleware: (getDefaultMiddleware) =>
      services
        ? getDefaultMiddleware({
            // Disable expensive dev-only middlewares that deeply traverse the entire state tree
            // on every dispatch. With large search results in state, these cause 70-90ms+ delays
            // per dispatch, creating significant UI jank during multi-query execution.
...
export const getPreloadedStore = async (services: AgentTracesServices) => {
  try {
