...
jest.mock('../../../components/container/bottom_container', () => ({
  BottomContainer: () => (
    <div data-test-subj="bottom-container">
      ...
      <div data-test-subj="agentTraces-tabs">
...
jest.mock('../../../application/utils/hooks/use_page_initialization', () => ({
  useInitPage: jest.fn().mockReturnValue({
    savedAgentTraces: { id: 'test-id', title: 'Test Agent Traces' },
  }),
}));
...
describe('TracesPage', () => {
  ...
  beforeEach(() => {
    const agentTracesServices = discoverPluginMock.createAgentTracesServicesMock();
    const agentTracesServicesMock = agentTracesServices as jest.MaybeMockedDeep<
      typeof agentTracesServices
    >;
    agentTracesServicesMock.uiSettings.get.mockImplementation((_, defaultValue) => defaultValue);

    (useOpenSearchDashboards as jest.Mock).mockReturnValue({
      services: agentTracesServicesMock,
    });
  });
