...
import { AgentTracesUrlGenerator } from './url_generator';
...
const appBasePath: string = 'xyz/app/agentTraces';
...
const setup = async ({ useHash = false }: SetupParams = {}) => {
  const generator = new AgentTracesUrlGenerator({
    appBasePath,
    useHash,
  });
...
describe('Agent Traces url generator', () => {
  test('can create a link to Agent Traces with no state and no saved object', async () => {
  ...
  test('can create a link to a saved object in Agent Traces', async () => {
    const { generator } = await setup();
