...
describe('area/to_expression.ts', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  it('should generate vislib expression for area chart when useVega is false', async () => {
    const mockState = {
      style: { addLegend: true, addTooltip: true, legendPosition: 'right', type: 'area' },
  ...
  it('should generate vega expression for area chart when useVega is true', async () => {
    const mockState = {
      style: { addLegend: true, addTooltip: true, legendPosition: 'right', type: 'area' },
      visualization: { someConfig: 'value' },
    };
