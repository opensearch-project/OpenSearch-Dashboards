 ...
 * dispatch `setQueryState`: the Redux query string is the results cacheKey (see
...
export const LogsQueryPanel: React.FC<LogsQueryPanelProps> = ({
  ...
  // Builder's live state, in a ref so per-keystroke edits don't re-render.
  const builderStateRef = useRef(initialParse.state);
  // Bumped on re-seed so PPLBuilder remounts and picks up the new initialState.
  const [builderKey, setBuilderKey] = useState(0);

  // Live code-editor text, so the Builder toggle can enable/disable itself.
  ...
  // Builder -> Code toggle without pushing to Redux (which would re-key results).
  const builderQueryRef = useRef(reduxQuery);
  // Snapshot of builder state on a Builder -> Code toggle, keyed by the query it
  // compiled to. buildPPL is lossy (fieldless metric, `auto` span, stale sort
  // don't survive parse(build(state))), so on Code -> Builder we restore this
  // verbatim iff the code is unchanged, else fall back to parsing the new text.
  ...
  // Mirror of `mode` for the external-sync effect, kept out of its deps so a mode
  // toggle doesn't re-run it and reset the in-progress builder draft.
  ...
  // reads it via queryString.getQuery().query, keeping the results cacheKey stable.
  ...
  const handleModeChange = useCallback(
    (newMode: LogsBuilderMode) => {
      ...
      if (newMode === 'code') {
        // Snapshot the full builder state so partial work survives an unedited
        // round-trip back (the parse(build(state)) path is lossy).
  ...
  return (
    <EuiPanel paddingSize="s" borderRadius="none" className="exploreQueryPanel">
      ...
      {/* `editors` must stay in ONE reconciliation slot across prompt and code
          modes: remounting `<QueryPanelEditor/>` mid-type drops the AI prompt and
          the query never runs. `showBuilder` is already false in prompt mode. */}
      <EuiFlexGroup
