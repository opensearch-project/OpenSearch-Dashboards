...
import * as redirectionHelpers from './utils/redirection_helpers';

// Mock the TopNavMenu component
...
// Mock the redirectToLogs function
jest.spyOn(redirectionHelpers, 'redirectToLogs').mockImplementation(jest.fn());
...
describe('TraceTopNavMenu', () => {
  ...
  it('renders the top nav menu with correct buttons', () => {
    ...
    expect(screen.getByTestId('viewLogsBtn')).toBeInTheDocument();
  ...
  it('calls redirectToLogs when View associated Logs button is clicked', () => {
    render(<TraceTopNavMenu {...defaultProps} />);

    // Click the View associated Logs button
    fireEvent.click(screen.getByTestId('viewLogsBtn'));

    // Check that redirectToLogs was called with the correct parameters
    expect(redirectionHelpers.redirectToLogs).toHaveBeenCalledWith(
      defaultProps.payloadData,
