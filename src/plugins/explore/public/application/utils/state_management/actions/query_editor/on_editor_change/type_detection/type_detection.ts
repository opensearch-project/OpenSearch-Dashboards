...
import { EditorLanguage, PatternUtils, MAX_QUERY_LENGTH } from './constants';
...
export class QueryTypeDetector {
  public static detect(query: string): DetectionResult {
  ...
  private static _checkPpl(query: string): ScoreResult {
    let score = 0;
  ...
  private static _checkKeyValue(query: string): ScoreResult {
    let score = 0;
    const reasons: string[] = [];
    const warnings: string[] = [];
  ...
  private static _checkNaturalLanguage(query: string): ScoreResult {
    let score = 0;
    const reasons: string[] = [];
    const warnings: string[] = [];
