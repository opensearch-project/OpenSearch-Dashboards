...
export function QueryError(props: { queryStatus: QueryStatus }) {
  ...
  if (props.queryStatus.status === QueryExecutionStatus.LOADING) {
  ...
  if (props.queryStatus.status === QueryExecutionStatus.READY) {
    let message;
  ...
  if (
    props.queryStatus.status === QueryExecutionStatus.UNINITIALIZED ||
    !props.queryStatus.body?.error
  ) {
    return null;
  }
