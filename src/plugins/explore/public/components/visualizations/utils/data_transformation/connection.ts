...
export const resolveConnectMode = (styles: StyleOptions): DisableMode =>
  styles.connectNullValues?.connectMode ?? DisableMode.Never;

export const resolveDisconnectMode = (styles: StyleOptions): DisableMode =>
  styles.disconnectValues?.disableMode ?? DisableMode.Never;

/**
 * Connect the nulls lying within threshold of the last valid point and leave the
 * rest of the run as a break.
 */
export const connectNullGaps = (
  ...
  fields.forEach((field) => {
    ...
    const flushBuffer = (endIndex: number) => {
      ...
      if (duration > 0) {
        const startValue = Number(rows[lastValidIndex][field]);
        const endValue = Number(rows[endIndex][field]);

        buffer.forEach((index) => {
          const gap = toTimestamp(rows[index][timeField]) - startTime;
          // apply linear interpolation
          if (gap <= maxGap) {
            rows[index][field] = startValue + (endValue - startValue) * (gap / duration);
          }
        });
      }
