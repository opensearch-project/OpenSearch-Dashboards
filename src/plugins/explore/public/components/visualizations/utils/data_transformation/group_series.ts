...
/**
 * Build and transform one dataset per color series
 */
...
/**
 * Split each series into its own dataset
 */

export const splitSeriesDatasets =
  ({
    valueFields,
    timeField,
    perSeries,
  ...
  (data: Array<Record<string, any>>): Array<Array<string[] | any[]>> => {
    ...
    return fields.map((field) => {
      const prepared = perSeries ? perSeries(field, data) : data;
      const headers = [timeField, field];
      return [headers, ...prepared.map((row) => headers.map((header) => row[header]))];
    });
  };
