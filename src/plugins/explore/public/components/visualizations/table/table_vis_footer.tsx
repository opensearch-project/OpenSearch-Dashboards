...
import { CalcType, VisColumn } from '../types';

export type TableFooterStyleControlsProps = StyleControlsProps<TableChartStyleControls> & {
  numericalColumns?: VisColumn[];
};

const CALCULATIONS: CalcType[] = ['total', 'last', 'average', 'min', 'max'];
...
export const TableFooterStyleControls: React.FC<TableFooterStyleControlsProps> = ({
  ...
  const [localCalculations, setLocalCalculations] = useState<
    Array<{ fields: string[]; calculation: CalcType }>
  ...
  const syncToParent = useCallback(
    (next: Array<{ fields: string[]; calculation: CalcType }>) => {
  ...
  const onAddCalculation = useCallback(() => {
    ...
    const usedCalculations = new Set<CalcType>(localCalculations.map((c) => c.calculation));
  ...
  const getCalculationOptionsForIndex = useCallback(
    (index: number) => {
      ...
      const usedByOthers = new Set<CalcType>(
        localCalculations.filter((_, i) => i !== index).map((c) => c.calculation)
      );
      const baseOptions: Array<{ value: CalcType; text: string }> = [
                    ...
                    <EuiFlexGroup gutterSize="s" alignItems="center">
                      <EuiFlexItem>
                        <EuiSelect
                          ...
                          onChange={(e) => onCalculationChange(index, e.target.value as CalcType)}
