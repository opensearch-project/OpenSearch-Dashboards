...
import { SavedExplore, SavedExploreAttributes } from './types';
...
export const getLegacyPropertiesFromSavedObject = (savedExplore: SavedExplore) => {
  ...
  try {
    ...
    console.warn('Failed to parse legacy state from saved object:', error);
    return {
      columns: [],
      sort: [],
    };
...
export const updateLegacyPropertiesInSavedObject = (
  ...
  try {
    ...
    console.warn('Failed to update legacy state in saved object:', error);
    return savedExplore;
  }
};
