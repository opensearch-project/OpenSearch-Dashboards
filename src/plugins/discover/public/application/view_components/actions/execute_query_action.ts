...
export const LANGUAGE_TOOLS: LanguageToolConfig[] = [
  { languageKey: 'kuery', displayName: 'DQL', toolName: 'execute_dql_query' },
  { languageKey: 'lucene', displayName: 'Lucene', toolName: 'execute_lucene_query' },
  { languageKey: 'PPL', displayName: 'PPL', toolName: 'execute_ppl_query' },
  { languageKey: 'SQL', displayName: 'OpenSearch SQL', toolName: 'execute_sql_query' },
];
...
export const buildToolDefinition = (cfg: LanguageToolConfig) => ({
  ...
  LANGUAGE FOR ${cfg.displayName.toUpperCase()} ONLY: This tool ALWAYS writes ${cfg.displayName}. You cannot change the query language yourself - it is controlled by the user on the page. if a request needs another language, only that language's execute-query tool can handle it.
  WHEN TO USE: 
    (1)Call this when the user wants to execute a ${cfg.displayName} query, filter, or search over the data shown in Discover, thereby changing what the user is looking at - regardless of how the user phrases it (imperative like "show/list/filter/display/find", or interrogative/analytical like "is there any error?", "which orders are over $500?", "what errors happened on web-01?").
    (2)USE this tool if running the query and letting the user see the matching rows fully answers user request. But you SHOULD use backend data-retrieval tools with the same time range from/to when you need the actual field values to analyze, compute, or generate a report: this ${cfg.toolName} tool only returns execution status (success/failure) and a result count, NOT the underlying data.
    ${
  ...
  parameters: {
    ...
    properties: {
      query: {
        type: 'string',
        description: `The ${cfg.displayName} query to set in the query bar (without time filters)`,
      },
      description: {
        type: 'string',
...
export const EXECUTE_QUERY_TOOL_DEFINITIONS = LANGUAGE_TOOLS.map(buildToolDefinition);
...
const createUnavailableHandler = (cfg: LanguageToolConfig) => async () => ({
  ...
  message:
    ...
    'Use the execute-query tool for the language the page is currently in. The query language is controlled by the user.',
...
export function registerAllDisabledExecuteQueryActions(
...
export function useExecuteQueryAction(
  ...
  useMount(() => {
    ...
    const enableTool = (cfg: LanguageToolConfig) =>
      registerAction({
        ...buildToolDefinition(cfg),
        available: 'enabled',
        handler: createExecuteHandler(cfg, services, data$, refetch$, queryComplete$),
      });
  ...
  useUnmount(() => {
    ...
    if (registerAction) {
      registerAllDisabledExecuteQueryActions(registerAction);
    }
  });
}
