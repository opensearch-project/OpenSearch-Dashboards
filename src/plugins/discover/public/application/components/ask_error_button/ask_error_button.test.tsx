...
describe('<AskErrorButton />', () => {
  ...
  it('renders nothing when chat is unavailable', () => {
    ...
    const { queryByTestId } = render(<AskErrorButton />);
    expect(queryByTestId('discoverAskAIForHelp')).toBeNull();
  });
  ...
  it('renders the link when chat is available', () => {
    setup();
    const { getByTestId } = render(<AskErrorButton />);
    expect(getByTestId('discoverAskAIForHelp')).toBeInTheDocument();
  });

  it('sends the error-specific message when an error is provided', () => {
    const { sendMessageWithWindow } = setup();
    const { getByTestId } = render(<AskErrorButton getError={() => 'boom parse error'} />);

    fireEvent.click(getByTestId('discoverAskAIForHelp'));
  ...
  it('sends the no-results message when there is no error', () => {
    const { sendMessageWithWindow } = setup();
    const { getByTestId } = render(<AskErrorButton getError={() => undefined} />);
