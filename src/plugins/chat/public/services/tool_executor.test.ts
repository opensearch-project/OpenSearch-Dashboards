...
describe('ToolExecutor', () => {
  ...
  describe('executeTool with confirmation', () => {
    ...
    it('falls back to the generic message when the service does not implement getRejectionMessage', async () => {
      mockAssistantActionService.isUserConfirmRequired.mockReturnValue(true);
      // Simulate a service (e.g. an older/alternate implementation) that has no
      // getRejectionMessage method at all - the call must not throw.
      const original = mockAssistantActionService.getRejectionMessage;
      // @ts-expect-error intentionally removing the method for this scenario
      mockAssistantActionService.getRejectionMessage = undefined;
      ...
      try {
        ...
        mockAssistantActionService.getRejectionMessage = original;
      }
    });
