...
const ChatMessagesComponent: React.FC<ChatMessagesProps> = ({
  ...
  // A string key so visibleDefaults only changes when the available tools change,
  // not on every toolDefinitions emission.
  const toolAvailabilityKey = useMemo(
    () =>
      JSON.stringify(
        STARTER_SUGGESTIONS.map((suggestion) => suggestion.requiredTool).filter((requiredTool) =>
  ...
  const visibleDefaults = useMemo(() => {
    const availableTools = new Set<string>(JSON.parse(toolAvailabilityKey));
    return STARTER_SUGGESTIONS.filter(
      (suggestion) => !suggestion.requiredTool || availableTools.has(suggestion.requiredTool)
    );
  }, [toolAvailabilityKey]);
