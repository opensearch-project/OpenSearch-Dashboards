...
import { DatasetMissingData } from '../../../common/datasets/lib';
import { DatasetGetFieldsOptions, IDatasetsApiClient } from '../../../common/datasets/types';

const API_BASE_URL: string = `/api/datasets/`;

export class DatasetsApiClient implements IDatasetsApiClient {
    ...
    return this.http
      ...
      .catch((resp: any) => {
        if (resp.body.statusCode === 404 && resp.body.attributes?.code === 'no_matching_indices') {
          throw new DatasetMissingData(resp.body.message);
        }
  ...
  getFieldsForTimePattern(options: DatasetGetFieldsOptions = {}) {
  ...
  getFieldsForWildcard(options: DatasetGetFieldsOptions = {}) {
