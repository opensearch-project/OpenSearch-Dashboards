...
describe('OpenSavedQueryFlyout', () => {
  ...
  describe('recent queries tab', () => {
    const mockOnRecentQueryRun = jest.fn();
    const recentQuery = {
      id: 'recent-1',
      // Must match the query string manager mock's current language, or the table filters it out.
      query: { query: 'source=logs', language: 'kuery' },
      time: 1700000000000,
    };

    const renderFlyout = (props = {}) => {
      const queryString = queryStringManagerMock.createSetupContract();
      queryString.getQueryHistory.mockReturnValue([recentQuery] as never);
      (getQueryService as jest.Mock).mockReturnValue({ queryString });
    ...
    // Legacy safety: Discover and the dashboards search bar omit the callback and must see exactly
    // today's flyout.
    it('is absent when onRecentQueryRun is not provided', async () => {
      renderFlyout();

      await waitFor(() => screen.getByText('Saved Query 1'));

      expect(tabNames()).toEqual(['Saved queries']);
      expect(screen.queryByTestId('recentQueryTable')).not.toBeInTheDocument();
    });

    it('is rendered directly after Saved queries when opted in', async () => {
      renderFlyout({ onRecentQueryRun: mockOnRecentQueryRun });

      await waitFor(() => screen.getByText('Saved Query 1'));

      expect(tabNames()).toEqual(['Saved queries', 'Recent queries']);
    });

    it('runs the selected recent query and closes the flyout', async () => {
      renderFlyout({ onRecentQueryRun: mockOnRecentQueryRun });

      await waitFor(() => screen.getByText('Saved Query 1'));
      fireEvent.click(screen.getByText('Recent queries'));

      fireEvent.click(await screen.findByTestId('action-run'));

      expect(mockOnRecentQueryRun).toHaveBeenCalledWith(recentQuery.query, undefined);
      expect(mockOnClose).toHaveBeenCalled();
    });

    it('hides the footer action button on the recent queries tab', async () => {
      renderFlyout({ onRecentQueryRun: mockOnRecentQueryRun });

      await waitFor(() => screen.getByText('Saved Query 1'));
      expect(screen.getByTestId('open-query-action-button')).toBeInTheDocument();

      fireEvent.click(screen.getByText('Recent queries'));

      // Recent rows carry their own Run/Copy actions, and nothing here is selectable.
      expect(screen.queryByTestId('open-query-action-button')).not.toBeInTheDocument();
    });

    it('does not refetch saved queries for the recent queries tab', async () => {
      renderFlyout({ onRecentQueryRun: mockOnRecentQueryRun });

      await waitFor(() => screen.getByText('Saved Query 1'));
      const fetchesBeforeSwitching = (mockSavedQueryService.getAllSavedQueries as jest.Mock).mock
        .calls.length;

      fireEvent.click(screen.getByText('Recent queries'));

      await waitFor(() => screen.getByTestId('recentQueryTable'));
      expect(mockSavedQueryService.getAllSavedQueries).toHaveBeenCalledTimes(
        fetchesBeforeSwitching
      );
    });
  });
});
