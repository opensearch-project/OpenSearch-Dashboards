...
import { extractFieldNames, extractFieldTypeMap } from '../../ppl_lint/lint_context_builder';
import type { LintFieldsCache } from '../../ppl_lint/lint_context_builder';

// The Discover editor's loadFields effect (query_editor.tsx:167-197) and the
// Explore editor's effect are intentionally identical: both await the index
// pattern for the active dataset, then stamp a LintFieldsCache with the field
// names AND the derived type map, keyed by dataset + data source id. The Explore
// hook variant is exercised end-to-end in use_query_panel_editor.test.ts
// ("loadFields effect caches field names and the type map"). The full Discover
// QueryEditor is too dependency-heavy to render in isolation, so these tests
// pin the exact async load/cancel/failure contract the Discover effect relies on
// — using the real extractors — so a regression in the shared cache shape or the
// stale-suppression behavior fails here.
...
async function runLoadFields(opts: {
  ...
  const { datasetId, dataSourceId, getIndexPattern, isCancelled = () => false } = opts;
