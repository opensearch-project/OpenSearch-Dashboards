...
describe('ppl_grammar_warmup', () => {
  ...
  it('should forward to warmUp even when version is old (cache handles version gating)', () => {
    ...
    expect(cache.warmUp).toHaveBeenCalledTimes(1);
    expect(cache.warmUp).toHaveBeenCalledWith(http, savedObjectsClient, 'ds-old', '2.17.0');
  });
  ...
  it('should warm up when version is not provided (needs lookup)', () => {
    ...
    expect(cache.warmUp).toHaveBeenCalledTimes(1);
    expect(cache.warmUp).toHaveBeenCalledWith(http, savedObjectsClient, 'ds-unknown', undefined);
  });
  ...
  it('should forward warmUp through rapid multi-datasource switching', () => {
    ...
    expect(cache.warmUp).toHaveBeenNthCalledWith(1, http, savedObjectsClient, 'ds-1', '3.6.0');
    expect(cache.warmUp).toHaveBeenNthCalledWith(2, http, savedObjectsClient, 'ds-2', '2.17.0');
    expect(cache.warmUp).toHaveBeenNthCalledWith(3, http, savedObjectsClient, 'ds-3', '3.7.0');
    expect(cache.warmUp).toHaveBeenNthCalledWith(4, http, savedObjectsClient, undefined, undefined);
  });
  ...
  it('should forward warmUp when switching to old-version datasource (cache auto-clears)', () => {
    ...
    expect(cache.warmUp).toHaveBeenCalledTimes(2);
    expect(cache.warmUp).toHaveBeenLastCalledWith(http, savedObjectsClient, 'ds-old', '2.17.0');
  });
});
