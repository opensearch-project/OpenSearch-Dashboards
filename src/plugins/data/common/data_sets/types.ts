...
// TODO: Temporary, use the ones from the correct source once reconciled
...
/**
 * Metadata for FEATURE type data structure
 */
...
export interface DatasetTypeConfig {
  ...
  getDataset: (path: DataStructure[]) => Dataset;
  getFields: (dataset: Dataset) => Promise<DatasetField[]>;
  config: {
    icon: EuiIconProps;
...
export interface DatasetManager {
  registerType: (props: DatasetTypeConfig) => void;
  getTypes: () => DatasetTypeConfig[];
  getType: (type: string) => DatasetTypeConfig;
  getDataSet: () => Dataset | undefined;
  setDataSet: (dataset: Dataset) => void;
  getRecentlyUsed: () => Dataset[];
}

export interface DatasetPathItem extends DataStructure {
  columnHeader: string;
  isLoadable?: boolean;
}
