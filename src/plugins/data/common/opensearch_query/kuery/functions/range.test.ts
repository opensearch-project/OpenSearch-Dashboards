  ...
  describe('range', () => {
    ...
    describe('toOpenSearchQuery', () => {
      test("should return an OpenSearch range query for the node's field and params", () => {
        ...
        const result = range.toOpenSearchQuery(node, indexPattern);
      ...
      test('should return an OpenSearch range query without an index pattern', () => {
        ...
        const node = nodeTypes.function.buildNode('range', 'bytes', { gt: 1000, lt: 8000 });
        const result = range.toOpenSearchQuery(node);
      ...
      test('should support date fields with a dateFormat provided', () => {
        ...
        const result = range.toOpenSearchQuery(node, indexPattern, config);
      ...
      test('should use a provided nested context to create a full field name', () => {
        ...
        const node = nodeTypes.function.buildNode('range', 'bytes', { gt: 1000, lt: 8000 });
        const result = range.toOpenSearchQuery(
          node,
          indexPattern,
          {},
