...
import { buildOpenSearchQuery } from './build_opensearch_query';
    ...
    it('should combine queries and filters from multiple query languages into a single OpenSearch bool query', () => {
      ...
      const expectedResult = {
        bool: {
          ...
          filter: [
            toOpenSearchQuery(fromKueryExpression('extension:jpg'), indexPattern),
            { match_all: {} },
          ],
          should: [],
          must_not: [],
      ...
      const result = buildOpenSearchQuery(indexPattern, queries, filters, config);
