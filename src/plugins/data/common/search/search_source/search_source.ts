...
export class SearchSource {
  ...
  /**
   * Get the data set of this SearchSource
   * @return {undefined|IndexPattern}
   */
  getDataSet() {
    const df = this.dependencies.df.get();
    if (!df) {
      return undefined;
    }
    return dataFrameToSpec(df);
  }
  ...
  private async fetchExternalSearch(searchRequest: SearchRequest, options: ISearchOptions) {
    ...
    if (this.getDataFrame() && this.getDataFrame()!.name === searchRequest.index.title) {
