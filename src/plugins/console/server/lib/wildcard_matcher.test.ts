...
function should(candidate: string, pattern: string, ...emptyVal: string[]) {
  if (!new WildcardMatcher(pattern, ...emptyVal).match(candidate)) {
    throw new Error(`Expected pattern ${[pattern, ...emptyVal]} to match ${candidate}`);
  }
}

function shouldNot(candidate: string, pattern: string, ...emptyVal: string[]) {
  if (new WildcardMatcher(pattern, ...emptyVal).match(candidate)) {
    throw new Error(`Expected pattern ${[pattern, ...emptyVal]} to not match ${candidate}`);
  }
}
