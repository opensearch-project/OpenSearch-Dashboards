...
export function registerNonMdsDataConnectionsRoute(router: IRouter) {
  router.get(
    ...
    async (context, request, response): Promise<any> => {
      try {
        const dataConnectionsresponse = await context.opensearch_dashboard_plugin.dashboardPluginClient
          .asScoped(request)
    ...
    async (context, request, response): Promise<any> => {
      ...
      try {
        ...
        if (dataSourceEnabled && dataSourceMDSId) {
          ...
          dataConnectionsresponse = await context.opensearch_dashboard_plugin.dashboardPluginClient
            .asScoped(request)
            .callAsCurrentUser('ppl.getDataConnections');
        }
        return response.ok({
