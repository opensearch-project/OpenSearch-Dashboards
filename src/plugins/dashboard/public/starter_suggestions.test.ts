...
describe('registerDashboardStarterSuggestions', () => {
  ...
  describe('a saved dashboard', () => {
    ...
    it('leads with a summarize card that brings a screenshot along', () => {
    ...
    it('caps the panel at three cards, dropping the defaults that do not fit', () => {
      const INVESTIGATE: StarterSuggestionItem = {
        id: 'investigate',
        icon: 'notebookApp',
        text: '/investigate an issue',
      };

      const capped = provider().getSuggestions({
        ...context,
        defaults: [ASK_DATA, EXPLAIN, INVESTIGATE],
      }) as StarterSuggestionItem[];

      expect(capped).toEqual([
        expect.objectContaining({ id: 'summarizeDashboard' }),
        ASK_DATA,
        EXPLAIN,
      ]);
    });
  ...
  describe('an unsaved draft', () => {
    ...
    it('finds the panels behind a _g that carries its own question mark', () => {
      ...
      expect(cards()).toEqual([
        expect.objectContaining({ id: 'summarizeDraftDashboard' }),
        ASK_DATA,
        EXPLAIN,
      ]);
    });
    ...
    it.each([
      ...
      expect(cards()).toEqual([
        expect.objectContaining({ id: 'buildDashboard' }),
        ASK_DATA,
        EXPLAIN,
      ]);
    });
