...
describe('parseResponseToQueryResult with language=PROMQL', () => {
  it('should read from instantHits, not hits.hits, when instantHits is present', () => {
    const response = {
      // Present but should be ignored — Labels is a nested object, not a
      // usable scalar variable value/label field.
      hits: {
        hits: [
          {
            _source: {
              Time: 1787123600000,
              Series: '{instance="node-exporter-1:9100"}',
              Labels: { instance: 'node-exporter-1:9100' },
              Value: 15.96,
            },
          },
        ],
        ...
        hits: [
          {
            _source: {
              Time: 1787123600000,
              Metric: '{instance="node-exporter-1:9100"}',
          ...
          },
          {
            _source: {
              Time: 1787123600000,
              Metric: '{instance="node-exporter-2:9100"}',
              instance: 'node-exporter-2:9100',
              Value: 15.97,
            },
          },
        ],
    ...
    expect(parseResponseToQueryResult(response, 'PROMQL')).toEqual({
      rows: [
        {
          Time: 1787123600000,
          Metric: '{instance="node-exporter-1:9100"}',
      ...
      fields: ['Time', 'Metric', 'instance', 'Value'],
      fieldTypes: {
        Time: 'number',
        Metric: 'string',
        instance: 'string',
  ...
  it('should return empty metadata when language is PROMQL but instantHits is absent (no fallback to hits.hits)', () => {
    const response = {
      hits: {
        hits: [{ _source: { Series: 'up{job="api"}', Value: 1 } }],
      },
    };
  ...
  it('should not read instantHits when language is not PROMQL', () => {
    const response = {
      hits: {
        hits: [{ _source: { Series: 'up{job="api"}', Value: 1 } }],
      },
      instantHits: {
        hits: [{ _source: { Time: 1, instance: 'node-exporter-1:9100', Value: 1 } }],
      },
    };

    expect(parseResponseToQueryResult(response, 'PPL')).toEqual({
      rows: [{ Series: 'up{job="api"}', Value: 1 }],
      fields: ['Series', 'Value'],
      fieldTypes: { Series: 'string', Value: 'number' },
    });
  });

  it('should return empty metadata when neither instantHits nor hits.hits are present', () => {
    expect(parseResponseToQueryResult(undefined, 'PROMQL')).toEqual({
      rows: [],
      fields: [],
      fieldTypes: {},
    });
...
describe('executeVariableQuery', () => {
  ...
  it('should parse PromQL responses via instantHits, not hits.hits', async () => {
    mockFetch.mockResolvedValueOnce({
      hits: {
        hits: [{ _source: { Series: 'up{job="api"}', Labels: { job: 'api' }, Value: 1 } }],
      },
      instantHits: {
        hits: [{ _source: { Time: 1, Metric: 'up{job="api"}', job: 'api', Value: 1 } }],
      },
    });
    ...
    const result = await executeVariableQuery(mockDataPlugin, {
      query: 'up',
      language: 'PROMQL',
      dataset: { id: 'prometheus_local', title: 'prometheus_local', type: 'PROMETHEUS' },
    });

    expect(result).toEqual({
      rows: [{ Time: 1, Metric: 'up{job="api"}', job: 'api', Value: 1 }],
      fields: ['Time', 'Metric', 'job', 'Value'],
      fieldTypes: { Time: 'number', Metric: 'string', job: 'string', Value: 'number' },
    });
  });
});
