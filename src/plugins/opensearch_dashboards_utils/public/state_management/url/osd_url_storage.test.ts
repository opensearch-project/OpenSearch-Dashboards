...
describe('osd_url_storage', () => {
  describe('getStateFromUrl & setStateToUrl', () => {
    ...
    it('should set expanded state to url', () => {
      let newUrl = setStateToOsdUrl('_s', state1, { useHash: false }, url);
      expect(newUrl).toMatchInlineSnapshot(
        `"http://localhost:5601/oxf/app/opensearch-dashboards#/yourApp?_s=(testArray:!(1,2,()),testNull:!n,testNumber:0,testObj:(test:'123'),testStr:'123')"`
      );
      ...
      expect(newUrl).toMatchInlineSnapshot(
        `"http://localhost:5601/oxf/app/opensearch-dashboards#/yourApp?_s=(test:'123')"`
      );
    ...
    it('should set hashed state to url', () => {
      let newUrl = setStateToOsdUrl('_s', state1, { useHash: true }, url);
      expect(newUrl).toMatchInlineSnapshot(
        `"http://localhost:5601/oxf/app/opensearch-dashboards#/yourApp?_s=h@a897fac"`
      );
      const retrievedState1 = getStateFromOsdUrl('_s', newUrl);
      expect(retrievedState1).toEqual(state1);
      ...
      expect(newUrl).toMatchInlineSnapshot(
        `"http://localhost:5601/oxf/app/opensearch-dashboards#/yourApp?_s=h@40f94d5"`
      );
      const retrievedState2 = getStateFromOsdUrl('_s', newUrl);
      expect(retrievedState2).toEqual(state2);
    });
    ...
    it('should set query to url with storeInHashQuery: false', () => {
      let newUrl = setStateToOsdUrl(
        ...
        'http://localhost:5601/oxf/app/opensearch-dashboards/yourApp'
      );
      expect(newUrl).toMatchInlineSnapshot(
        `"http://localhost:5601/oxf/app/opensearch-dashboards/yourApp?_a=(tab:other)"`
      );
  ...
  describe('urlControls - scoped history integration', () => {
    ...
    it('should flush async url updates', async () => {
      const pr1 = urlControls.updateAsync(() => '/app/opensearch-dashboards/1', false);
      const pr2 = urlControls.updateAsync(() => '/app/opensearch-dashboards/2', false);
      const pr3 = urlControls.updateAsync(() => '/app/opensearch-dashboards/3', false);
      expect(getCurrentUrl()).toBe('/app/opensearch-dashboards/');
      expect(urlControls.flush()).toBe('/app/opensearch-dashboards/3');
      expect(getCurrentUrl()).toBe('/app/opensearch-dashboards/3');
      await Promise.all([pr1, pr2, pr3]);
      expect(getCurrentUrl()).toBe('/app/opensearch-dashboards/3');
    });
    ...
    it('flush() should return undefined, if no url updates happened', () => {
      expect(urlControls.flush()).toBeUndefined();
      urlControls.updateAsync(() => '/app/opensearch-dashboards/1', false);
      urlControls.updateAsync(() => '/app/opensearch-dashboards/', false);
      expect(urlControls.flush()).toBeUndefined();
    });
  });
  ...
  describe('getRelativeToHistoryPath', () => {
    ...
    it('should extract path relative to browser history with basename', () => {
      ...
      const history2 = createBrowserHistory({ basename: '/oxf/app/opensearch-dashboards/' });
