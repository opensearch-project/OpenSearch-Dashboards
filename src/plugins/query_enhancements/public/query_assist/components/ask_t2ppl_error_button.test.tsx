...
describe('<AskT2pplErrorButton />', () => {
  ...
  it('renders as an EuiLink by default', () => {
    ...
    const { getByTestId } = render(
      <AskT2pplErrorButton chatService={chatService} error={new Error('boom')} />
    );
    // EuiLink with an onClick renders a <button> too, so distinguish by class.
    expect(getByTestId('queryAssistAskAIForHelp').className).toContain('euiLink');
  });

  it('renders as an EuiButton when as="button"', () => {
    const { chatService } = createChatService();
    const { getByTestId } = render(
      <AskT2pplErrorButton as="button" chatService={chatService} error={new Error('boom')} />
    );
    expect(getByTestId('queryAssistAskAIForHelp').className).toContain('euiButton');
  });
  ...
  it('uses error.message as the reason for a plain Error and includes the question', () => {
    ...
    fireEvent.click(getByTestId('queryAssistAskAIForHelp'));
  ...
  it('omits the question phrase when no question is provided', () => {
    const { chatService, sendMessageWithWindow } = createChatService();
    const { getByTestId } = render(
      <AskT2pplErrorButton chatService={chatService} error={new Error('failed')} />
    );
  ...
  it('uses the agent reason and extracted details for an AgentError', () => {
    ...
    const { getByTestId } = render(
      <AskT2pplErrorButton chatService={chatService} error={agentError} question="q" />
    );
  ...
  it('does not throw when no chatService is provided', () => {
    const { getByTestId } = render(<AskT2pplErrorButton error={new Error('boom')} />);
    expect(() => fireEvent.click(getByTestId('queryAssistAskAIForHelp'))).not.toThrow();
  });
});
