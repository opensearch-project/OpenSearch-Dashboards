...
class TestManager extends BaseConnectionManager {
  ...
  constructor(
    ...
    super();
    this.handleRequestSpy = handleRequestSpy;
  }
  ...
  handleRequest(
    ...
    return this.handleRequestSpy(context, request);
  }
}
...
describe('Resource Routes', () => {
  ...
  it('should connect to manager based on data connection type', async () => {
    const handleRequestSpy = jest.fn().mockResolvedValue({ message: 'succeed' });
    const manager = new TestManager(handleRequestSpy);
    jest
  ...
  it('should return error when get resources failed', async () => {
    const handleRequestSpy = jest.fn().mockRejectedValue({ message: 'failed' });
    const manager = new TestManager(handleRequestSpy);
    jest
      .spyOn(resourceManagerService, 'getManager')
      .mockImplementation((type: string) => (type === DATA_CONNECTION_TYPE ? manager : undefined));
