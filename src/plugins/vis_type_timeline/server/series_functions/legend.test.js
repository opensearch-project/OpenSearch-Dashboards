...
describe('legend.js', () => {
  ...
  it('should create the _global object if it does not exist', () => {
    ...
    return invoke(fn, [seriesList, 'nw', 3, true, 'YYYY']).then((r) => {
      expect(r.output.list[0]._global).toEqual({
  ...
  it('should provide default values for time axis display', () => {
    return invoke(fn, [seriesList, 'nw', 3]).then((r) => {
      expect(r.output.list[0]._global.legend.showTime).toEqual(true);
      expect(r.output.list[0]._global.legend.timeFormat).toEqual('MMMM Do YYYY, HH:mm:ss.SSS');
    });
  });

  it('should hide the legend is position is false', () => {
    return invoke(fn, [seriesList, false]).then((r) => {
      expect(r.output.list[0]._global.legend.show).toEqual(false);
      expect(r.output.list[0]._global.legend.showTime).toEqual(false);
    });
  });
