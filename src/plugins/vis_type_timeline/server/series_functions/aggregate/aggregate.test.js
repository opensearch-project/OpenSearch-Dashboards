...
describe('aggregate', () => {
  ...
  it('first', () => {
    return invoke(fn, [seriesList, 'first']).then((r) => {
      expect(_.map(r.output.list[1].data, 1)).toEqual([100, 100, 100, 100]);
    });
  });

  it('last', () => {
    return invoke(fn, [seriesList, 'last']).then((r) => {
      expect(_.map(r.output.list[1].data, 1)).toEqual([20, 20, 20, 20]);
    });
  });
  ...
  it('sum', () => {
    return invoke(fn, [seriesList, 'sum']).then((r) => {
      expect(_.map(r.output.list[1].data, 1)).toEqual([220, 220, 220, 220]);
    });
  });

  it('cardinality', () => {
    return invoke(fn, [seriesList, 'cardinality']).then((r) => {
      expect(_.map(r.output.list[1].data, 1)).toEqual([3, 3, 3, 3]);
    });
  });

  it('avg', () => {
    return invoke(fn, [seriesList, 'avg']).then((r) => {
      expect(_.map(r.output.list[1].data, 1)).toEqual([55, 55, 55, 55]);
    });
  });
});
