...
describe('versionSatisfies', function () {
  it('returns false if versions are not strings', () => {
    ...
    expect(versionSatisfies('a')).toEqual(false);
    // @ts-ignore to test wrong args
    expect(versionSatisfies('a', 1.2)).toEqual(false);
    // @ts-ignore to test wrong args
    expect(versionSatisfies(1.2, 'a')).toEqual(false);
    // @ts-ignore to test wrong args
    expect(versionSatisfies(1.2, '1.2')).toEqual(false);
  });

  it('returns true if versions are exact match', () => {
    expect(versionSatisfies('3.14.159', '3.14.159')).toEqual(true);
  });
});
