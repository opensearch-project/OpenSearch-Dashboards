...
import { applyCspModifications, CspModificationRule } from './utils';
...
describe('applyCspModifications', () => {
  ...
  describe('add action', () => {
    ...
    it('should add value-less directive with empty values array', () => {
      const modifications: CspModificationRule[] = [
        { directive: 'block-all-mixed-content', action: 'add', values: [] },
      ];
      const result = applyCspModifications(defaultRules, modifications);
    ...
    it('should not add duplicate values', () => {
      const modifications: CspModificationRule[] = [
        { directive: 'script-src', action: 'add', values: ["'self'"] },
      ];
      const result = applyCspModifications(defaultRules, modifications);
  ...
  describe('remove action', () => {
    ...
    it('should handle removing from non-existent directive gracefully', () => {
      const modifications: CspModificationRule[] = [
        { directive: 'img-src', action: 'remove', values: ["'self'"] },
      ];
      const result = applyCspModifications(defaultRules, modifications);
      expect(result).toBe(
        "script-src 'unsafe-eval' 'self'; worker-src blob: 'self'; style-src 'unsafe-inline' 'self'"
    ...
    it('should handle remove with empty values array', () => {
      const modifications: CspModificationRule[] = [
        { directive: 'script-src', action: 'remove', values: [] },
      ];
      const result = applyCspModifications(defaultRules, modifications);
      expect(result).toBe(
        "script-src 'unsafe-eval' 'self'; worker-src blob: 'self'; style-src 'unsafe-inline' 'self'"
  ...
  describe('set action', () => {
    ...
    it('should handle set with empty array (value-less directive)', () => {
      const modifications: CspModificationRule[] = [
        { directive: 'upgrade-insecure-requests', action: 'set', values: [] },
      ];
      const result = applyCspModifications(defaultRules, modifications);
      expect(result).toContain('upgrade-insecure-requests');
    });
  ...
  describe('edge cases', () => {
    ...
    it('should handle data: and blob: schemes', () => {
      const modifications: CspModificationRule[] = [
        { directive: 'img-src', action: 'add', values: ['data:', 'blob:'] },
      ];
      const result = applyCspModifications(defaultRules, modifications);
