...
describe('DynamicConfigControlledUiSettingsWrapper', () => {
  ...
  const buildWrapperInstance = (
    ...
    const wrapperInstance = new DynamicConfigControlledUiSettingsWrapper(
      permissionEnabled,
  ...
  describe('#update', () => {
    ...
    it('should only consider global ui settings updates', async () => {
      const wrapperClient = buildWrapperInstance(true, false, false);
      const attributes = { 'csv:quoteValues': 'true' };
    ...
    it('should update global settings when user is dashboard admin even globalScopeEditable is disabled', async () => {
      const wrapperClient = buildWrapperInstance(true, true, false);
      const attributes = { 'csv:quoteValues': 'true' };
      const mockResponse = {
        id: '3.0.0',
        type: 'config',
  ...
  describe('#bulkUpdate', () => {
    ...
    it('should throw error when trying to bulk update global settings', async () => {
      const attributes = { 'csv:quoteValues': 'true' };
      const wrapperClient = buildWrapperInstance(true, false, false);
      const objects = [
        {
