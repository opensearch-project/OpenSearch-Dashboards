 ...
 */
 ...
 export const getNodeId = async (
   ...
   try {
     ...
     let path = `nodes.*.attributes.di_number,nodes.*.attributes.box_type`;
     ...
     console.log('before delete', JSON.stringify(nodeIds));
     nodeIds.forEach((id) => {
       if ('warm' === get(nodes[id], `attributes.box_type`, null)) {
         delete nodes[id];
       }
     });

     console.log('after delete', JSON.stringify(nodeIds));
     ...
     const sharedClusterId = get(nodes[nodeIds[0]], `attributes.di_number`, null);
     if (sharedClusterId === null ) {
       return null
     }

     const findIfNodeDINumberDoesnMatch = state.body.nodes.find(
       (node: any) => sharedClusterId !== get(node, `attributes.di_number`, null)
     );
