...
describe('plugins discovery system', () => {
  ...
  it('return an error when the manifest file is not accessible', async () => {
    ...
    const standardizedErrorPath = standardize(errorPath, false, false, false);
    expect(errors).toEqual([
      `Error: EACCES: permission denied, open '${standardizedErrorPath}' (missing-manifest, ${errorPath})`,
    ]);
  });
