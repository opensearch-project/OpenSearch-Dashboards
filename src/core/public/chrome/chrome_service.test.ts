...
describe('keyboard shortcuts', () => {
  ...
  it('registers left navbar toggle keyboard shortcut correctly', async () => {
    const startDeps = defaultStartDeps();
    const registerSpy = jest.spyOn(startDeps.keyboardShortcut, 'register');
  ...
  it('keyboard shortcut clicks collapsibleNavShrinkButton when available', async () => {
    const startDeps = defaultStartDeps();
    const registerSpy = jest.spyOn(startDeps.keyboardShortcut, 'register');

    // Create mock button element
    const mockButton = document.createElement('button');
    mockButton.setAttribute('data-test-subj', 'collapsibleNavShrinkButton');
    ...
    const shortcutCall = registerSpy.mock.calls.find((call) => call[0].id === 'toggle_left_navbar');
  ...
  it('keyboard shortcut clicks toggleNavButton when collapsibleNavShrinkButton is not available', async () => {
    const startDeps = defaultStartDeps();
    const registerSpy = jest.spyOn(startDeps.keyboardShortcut, 'register');

    // Create mock toggle button element (fallback)
    const mockToggleButton = document.createElement('button');
    mockToggleButton.setAttribute('data-test-subj', 'toggleNavButton');
    const clickSpy = jest.spyOn(mockToggleButton, 'click');
  ...
  it('keyboard shortcut prefers collapsibleNavShrinkButton over toggleNavButton', async () => {
    const startDeps = defaultStartDeps();
    const registerSpy = jest.spyOn(startDeps.keyboardShortcut, 'register');

    // Create both button elements
    const mockShrinkButton = document.createElement('button');
    mockShrinkButton.setAttribute('data-test-subj', 'collapsibleNavShrinkButton');
    const shrinkClickSpy = jest.spyOn(mockShrinkButton, 'click');
    document.body.appendChild(mockShrinkButton);

    const mockToggleButton = document.createElement('button');
    mockToggleButton.setAttribute('data-test-subj', 'toggleNavButton');
    const toggleClickSpy = jest.spyOn(mockToggleButton, 'click');
    document.body.appendChild(mockToggleButton);
    ...
    // Should click the shrink button, not the toggle button
    expect(shrinkClickSpy).toHaveBeenCalledTimes(1);
    expect(toggleClickSpy).not.toHaveBeenCalled();
  });

  it('keyboard shortcut does nothing when no nav buttons are available', async () => {
    const startDeps = defaultStartDeps();
    const registerSpy = jest.spyOn(startDeps.keyboardShortcut, 'register');
    ...
    // Should not throw error when no buttons are available
    expect(() => executeFunction()).not.toThrow();
  });

  it('keyboard shortcut has correct metadata', async () => {
    const startDeps = defaultStartDeps();
    const registerSpy = jest.spyOn(startDeps.keyboardShortcut, 'register');
    ...
    expect(shortcutConfig.id).toBe('toggle_left_navbar');
    expect(shortcutConfig.pluginId).toBe('core');
    expect(shortcutConfig.name).toBe('Toggle Left Navbar');
    expect(shortcutConfig.category).toBe('Panel / Layout');
    expect(shortcutConfig.keys).toBe('shift+b');
    expect(typeof shortcutConfig.execute).toBe('function');
  });

  it('does not register keyboard shortcut when keyboardShortcut service is not available', async () => {
    const startDeps = defaultStartDeps();
    // Remove keyboard shortcut service
    startDeps.keyboardShortcut = undefined as any;

    // Should not throw error when keyboardShortcut service is not available
    await expect(start({ startDeps })).resolves.toBeDefined();
  });
});
