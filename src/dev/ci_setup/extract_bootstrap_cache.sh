...
bootstrapCache="$HOME/.opensearch-dashboards/bootstrap_cache/$targetBranch.tar"
...
if [ -f "$bootstrapCache" ]; then
  ...
  branchBootstrapCache="$HOME/.opensearch-dashboards/bootstrap_cache/$(jq -r .branch package.json).tar"
