...
export class Cache {
  ...
  constructor(config: { dir: string; log?: Writable; pathRoot: string; prefix: string }) {
    if (!Path.isAbsolute(config.pathRoot)) {
      throw new Error('an absolute path is required for the cache to resolve paths relative to');
    }
    ...
    // `timer.unref` is undefined in jest
  ...
  private getKey(path: string) {
    const relativePath =
      Path.sep !== PATH_SEPARATOR
        ? Path.relative(this.pathRoot, path).split(Path.sep).join(PATH_SEPARATOR)
