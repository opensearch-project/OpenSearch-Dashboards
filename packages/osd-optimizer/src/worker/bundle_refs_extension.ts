 ...
 * @notice
 *
 * This module was heavily inspired by the externals extension that ships with webpack@97d58d31
...
export class BundleRefsExtension {
  ...
  /**
   * Called by webpack when the extension is passed in the webpack config
   */
  public apply(compiler: webpack.Compiler) {
    // called whenever the compiler starts to compile, passed the params
    // that will be used to create the compilation
    compiler.hooks.compile.tap('BundleRefsExtension', (compilationParams: any) => {
      ...
      compilationParams.normalModuleFactory.hooks.factory.tap(
        'BundleRefsExtension/normalModuleFactory/factory',
    ...
    compiler.hooks.compilation.tap('BundleRefsExtension/getRequiredBundles', (compilation) => {
      ...
      compilation.hooks.additionalAssets.tap('BundleRefsExtension/watchManifest', () => {
        compilation.fileDependencies.add(manifestPath);
      });

      compilation.hooks.finishModules.tapPromise(
        'BundleRefsExtension/finishModules',
        async (modules) => {
  ...
  private async maybeReplaceImport(context: string, request: string) {
    ...
    for (const ref of possibleRefs) {
      ...
      if (!this.allowedBundleIds.has(ref.bundleId)) {
        throw new Error(
          `import [${request}] references a public export of the [${ref.bundleId}] bundle, but that bundle is not in the "requiredExtensions" or "requiredBundles" list in the extension manifest [${this.bundle.manifestPath}]`
        );
      }
