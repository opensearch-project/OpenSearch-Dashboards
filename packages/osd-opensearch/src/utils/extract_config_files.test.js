...
test('returns config with local paths', () => {
  const config = extractConfigFiles(['path=/data/foo.yml'], '/opensearch');
...
test('ignores non-paths', () => {
  const config = extractConfigFiles(['foo=bar', 'foo.bar=baz'], '/opensearch');
...
test('ignores directories', () => {
  fs.existsSync = () => true;
  const config = extractConfigFiles(['path=/data/foo.yml', 'foo.bar=/data/bar'], '/opensearch');
