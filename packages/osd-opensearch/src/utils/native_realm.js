...
exports.NativeRealm = class NativeRealm {
  constructor({ opensearchPassword, port, log = defaultLog, ssl = false, caCert }) {
    this._client = new Client({
      node: `${ssl ? 'https' : 'http'}://opensearch:${opensearchPassword}@localhost:${port}`,
      ssl: ssl
        ? {
            ca: caCert,
  ...
  async setPassword(username, password = this._opensearchPassword, retryOpts = {}) {
