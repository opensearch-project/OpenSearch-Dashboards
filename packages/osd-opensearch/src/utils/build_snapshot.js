...
exports.buildSnapshot = async ({ license, sourcePath, log, platform = os.platform() }) => {
  ...
  if (exitCode > 0) {
    throw createCliError('unable to build OpenSearch');
  }

  const archivePattern = `distribution/archives/${task}/build/distributions/opensearch-*.${ext}`;
  const opensearchArchivePath = findMostRecentlyChanged(path.resolve(sourcePath, archivePattern));

  if (!opensearchArchivePath) {
    throw createCliError('could not locate OpenSearch distribution');
  }

  return opensearchArchivePath;
};
