...
const { BASE_PATH, OPENSEARCH_CONFIG, OPENSEARCH_KEYSTORE_BIN } = require('../paths');
...
exports.installArchive = async function installArchive(archive, options = {}) {
  ...
  const tmpdir = path.resolve(installPath, 'OPENSEARCH_TMPDIR');
  ...
  if (license !== 'oss') {
    ...
    await configureKeystore(installPath, log, bundledJDK, [
      ...
      ...parseSettings(opensearchArgs, { filter: SettingsFilter.SecureOnly }),
    ]);
  }
...
/**
 * Appends single line to opensearch.yml config file
 *
...
async function appendToConfig(installPath, key, value) {
  fs.appendFileSync(path.resolve(installPath, OPENSEARCH_CONFIG), `${key}: ${value}\n`, 'utf8');
}
 ...
 * @param {Array<[string, string]>} secureSettings List of custom OpenSearch secure settings to
...
async function configureKeystore(
  ...
  await execa(OPENSEARCH_KEYSTORE_BIN, ['create'], { cwd: installPath, env });
  ...
  for (const [secureSettingName, secureSettingValue] of secureSettings) {
    ...
    await execa(OPENSEARCH_KEYSTORE_BIN, ['add', secureSettingName, '-x'], {
