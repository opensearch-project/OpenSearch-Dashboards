...
export async function updateVersions({
  ...
  if (extensionVersion && !/^\d+(\.\d+){2,}/.test(extensionVersion))
    throw createFailError('The extension version should start with #.#.# where # are numbers');

  if (compatibilityVersion && !/^\d+(\.\d+){2,}/.test(compatibilityVersion))
    throw createFailError(
      `The extension's compatibility version should start with #.#.# where # are numbers`
    );
