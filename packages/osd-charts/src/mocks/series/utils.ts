 ...
 * @internal
 */
export const getFilledNullData = (data: DataSeriesDatum[]): (number | undefined)[] => data.filter(({ y1 }) => y1 === null).map(({ filled }) => filled && filled.y1);

/**
 * Helper function to return array of rendered y1 values
 * @internal
 */
export const getFilledNonNullData = (data: DataSeriesDatum[]): (number | undefined)[] => data.filter(({ y1 }) => y1 !== null).map(({ filled }) => filled && filled.y1);
