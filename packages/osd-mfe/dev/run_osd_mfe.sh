...
#
# Launch the integrated MFE environment: a SECOND OpenSearch Dashboards instance
# booted with `--mfe` on :5602 (MFE_OSD_PORT), whose UI is loaded from Module
# Federation remotes served by the origin (:8080), while the old-way instance on
# :5601 keeps running untouched as the regression reference.
#
# Topology (see packages/osd-mfe/README.md):
...
# origin and/or CDN).
#
# CSP allow-list: the shipped server config derives it from the bootstrap /
# shared-deps URLs and adds the CDN origin from
# `opensearchDashboards.mfe.cdnOrigin`, widening script-src/worker-src itself
# (see src/legacy/ui/ui_render/{utils.js:buildMfeCspRules,ui_render_mixin.js}).
# So we only point the config at the CDN origin here — no more temp-yaml csp.rules.
#
# CDN: the dynamic registry can be repointed so plugin remoteEntry.js +
...
# Dev URL-override security gate. By default this is left UNSET so the server
# resolves it from dev mode (the canonical resolveAllowOverride: explicit
# boolean wins, else dev => true / prod => false). Launched with --dev,
...
# Server-side per-tenant resolution: the OSD server resolves the registry
# SERVER-SIDE when `opensearchDashboards.mfe.registryPath` points at the
# on-disk `schemaVersion: 1` registry document. With this set, the rendered
# HTML carries the projected `BootManifest` inline (under `__osdMfe__`) and
# the browser bootstrap consumes it DIRECTLY — no `/registry` HTTP fetch from
# the browser (the invariant the dual-path gate's verify_mfe_render
# locks). When the env var is unset, the line is OMITTED entirely so OSD
# falls back to the browser-fetch path (registryUrl) — same posture as a
# plain CI checkout without a workspace registry.
...
cat >"$MFE_CONFIG" <<YAML
# Auto-generated by run_osd_mfe.sh. Do not edit by hand.
server.port: $PORT

# MFE mode (see packages/osd-mfe/README.md).
...
# The CDN origin serving plugin remoteEntry.js + chunks. The
