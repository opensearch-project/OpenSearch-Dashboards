  ...
  new RunWithCommands({
    ...
    .command({
      ...
      async run({ log, flags }) {
        ...
        const pluginDir = await findOpenSearchDashboardsJson(fullpath());
        if (!pluginDir) {
          throw createFailError(
            `Unable to find OpenSearch Dashboards Platform plugin in [${fullpath()}] or any of its parent directories. Has it been migrated properly? Does it have a opensearch_dashboards.json file?`
          );
        }
    ...
    .command({
      ...
      async run({ log, flags }) {
        ...
        try {
          dashboardsPackage = await import(Path.join(fullpath(), '../../package.json'));
        } catch (ex) {
        ...
        let pluginPackage;
        try {
          pluginPackage = await import(Path.join(fullpath(), 'package.json'));
        } catch (ex) {
        ...
        let manifestFile;
        try {
          manifestFile = await import(Path.join(fullpath(), 'opensearch_dashboards.json'));
        } catch (ex) {
