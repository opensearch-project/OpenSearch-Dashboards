...
#   :8080  origin (harness/registry_server.js)      — /registry + /mfe + /shared-deps + /bootstrap (CORS)
#
...
if origin_ok; then
  ...
  log "Origin missing/stale /bootstrap route — (re)starting registry_server.js on :$ORIGIN_PORT"
  pkill -f "harness/registry_server.js" 2>/dev/null || true
  sleep 1
  ( node "$SCRIPT_DIR/registry_server.js" "$ORIGIN_PORT" >"$ORIGIN_LOG" 2>&1 & )
...
cat >"$MFE_CONFIG" <<YAML
# Auto-generated by harness/run_osd_mfe.sh (Phase 3, Story 5; Phase 5, Story 4). Do not edit by hand.
server.port: $PORT
