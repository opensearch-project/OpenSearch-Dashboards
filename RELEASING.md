...
Release notes are automatically generated by the [opensearch-build release automation](https://github.com/opensearch-project/opensearch-build/blob/main/jenkins/release-workflows/release-notes-generate.jenkinsfile):
- AI-powered generation analyzes commit history and creates structured release notes
- Automation creates a PR to the `release-notes/` directory with format `opensearch-dashboards.release-notes-<version>.md`
- The PR is created by `opensearch-ci` bot on branch `release-chores/release-notes-<version>`
- RM should review the generated content, add release highlights if needed, and merge the PR
