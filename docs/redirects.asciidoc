...
--
[role="exclude",id="monitoring-xpack-opensearch-dashboards"]
== Configuring monitoring in {osd}

See <<monitoring-opensearch-dashboards>>.


[role="exclude",id="settings-xpack-osd"]
...
[role="exclude",id="creating-df-osd"]
== Creating {transforms}
...
Using the `opensearch_dashboards_dashboard_only_user` role is deprecated.
Use <<opensearch-dashboards-feature-privileges,feature privileges>> instead.
...
== Reporting from OpenSearchDashboards
...
Watcher error reports have been removed and replaced with OpenSearchDashboards's <<apm-alerts,alerting and actions>> feature.
...
This content has moved. See <<known-opensearch-dashboards-plugins>>.
