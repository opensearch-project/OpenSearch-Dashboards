...
For private IP addresses, see https://www.elastic.co/blog/enriching-opensearch-data-geo-ips-internal-private-ip-addresses[Enriching data with GeoIPs from internal, private IP addresses].

[discrete]
=== Upload data with GDAL

https://www.gdal.org/[GDAL] (Geospatial Data Abstraction Library) contains command line tools that can convert geospatial data between 75 different geospatial file formats and index that geospatial data into {opensearch}.
See https://www.elastic.co/blog/how-to-ingest-geospatial-data-into-opensearch-with-gdal[Ingest geospatial data into OpenSearch with GDAL] for details.
