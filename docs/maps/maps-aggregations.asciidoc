...
When using aggregations, the documents stay in OpenSearch and only the calculated values for each group are returned to your computer.
...
See the <<maps-add-opensearch-layer, Getting started>> tutorial for more details on configuring the layers.
...
To get this data, {opensearch} first groups your data using a {ref}/search-aggregations-bucket-terms-aggregation.html[terms aggregation],
...
Point to point uses an {opensearch} {ref}/search-aggregations-bucket-terms-aggregation.html[terms aggregation] to group your documents by destination.
...
A term join uses a shared key to combine vector features, the left source, with the results of an {opensearch} terms aggregation, the right source.
...
The right source uses the OpenSearchDashboards sample data set "Sample web logs".
