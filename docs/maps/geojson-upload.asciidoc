...
data files directly into {opensearch}, and then use them as layers
in the map.  You can also use the GeoJSON data in the broader OpenSearchDashboards ecosystem,
...
the OpenSearch responses are shown on the *Layer add panel* and the indexed data
appears on the map. The geospatial data on the map
should be identical to the locally-previewed data, but now it's indexed data from OpenSearch.
