...
[float]
==== Before you begin
You must have a {osd} instance up and running.

If you are in an environment where you have multiple {osd} instances in a cluster, see the <<code-multiple-opensearch-dashboards-instances-config, config instructions for multiple OpenSearchDashboards instances>>.

[float]
==== Enable Code app
While in beta, you can turn on *Code* by adding the following line to `opensearch_dashboards.yaml`:
