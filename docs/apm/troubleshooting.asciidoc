...
your proposed changes in the https://github.com/elastic/opensearch-dashboards[OpenSearchDashboards repository].
...
If you're not outputting data directly from APM Server to OpenSearch (perhaps you're using Logstash or Kafka),
...
{apm-server-ref}/opensearch-output.html#index-option-opensearch[here].
If you change the default, you must also configure the `setup.template.name` and `setup.template.pattern` options.
See {apm-server-ref}/configuration-template.html[Load the OpenSearch index template].
If the OpenSearch index template has already been successfully loaded to the index,
...
<<apm-settings-osd,`xpack.apm.ui.transactionGroupBucketSize`>>.
...
In OpenSearch, index templates are used to define settings and mappings that determine how fields should be analyzed.
