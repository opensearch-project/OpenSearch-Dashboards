...
[role="screenshot"]
image::apm/images/global-filters.png[Global filters available in the APM app in OpenSearchDashboards]

[float]
===== Global time range

The <<set-time-filter,global time range filter>> in {osd} restricts APM data to a specific time period.
...
Similar to the query bar in {opensearch-dashboards-ref}/discover.html[Discover],
...
[role="screenshot"]
image::apm/images/local-filter.png[Local filters available in the APM app in OpenSearchDashboards]