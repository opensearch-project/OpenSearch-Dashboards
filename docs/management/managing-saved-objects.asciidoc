...
The `Saved Objects Management` {osd} privilege is required to access the *Saved Objects* UI.

To add the privilege, open the menu, then click *Stack Management > Roles*.

NOTE:
Granting access to Saved Objects Management will authorize users to manage all saved objects in {osd}, including objects that are managed by applications they may not otherwise be authorized to access.
...
{osd} instances. This action is useful when you
...
saved objects already in {osd} are overwritten.
