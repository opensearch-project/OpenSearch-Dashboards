...
Whenever possible,
{osd} uses the same field type for display as {opensearch}.  However, a few field types
{opensearch} supports are not available in {osd}. Use field formatters to customize how your
fields are displayed in OpenSearchDashboards, regardless of how they are stored in {opensearch}.

OpenSearchDashboards provides these field formatters:
...
the <<numeral, Elastic numeral pattern>> syntax that {osd} maintains.
...
Scripted fields compute data on the fly from the data in your {opensearch} indices. The data is shown on
the Discover tab as part of the document data, and you can use scripted fields in your visualizations. You query scripted fields with the <<kuery-query, {osd} query language>>, and can filter them using the filter bar. The scripted field values are computed at query time, so they aren't indexed and cannot be searched using the {osd} default
query language.

WARNING: Computing data on the fly with scripted fields can be very resource intensive and can have a direct impact on
{osd} performance. Keep in mind that there's no built-in validation of a scripted field. If your scripts are
buggy, you'll get exceptions whenever you try to view the dynamically generated data.

When you define a scripted field in {osd}, you have a choice of the {ref}/modules-scripting-expression.html[Lucene expressions] or the 
...
https://www.elastic.co/blog/using-painless-opensearch-dashboards-scripted-fields[Using Painless in {osd} scripted fields]
...
For more information about scripted fields in {opensearch}, see
