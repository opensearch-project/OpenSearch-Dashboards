...
{osd} makes it easy for you to create a rollup job by walking you through
...
In this example, you want data that is older than 7 days in the target index pattern `opensearch_dashboards_sample_data_logs`
...
. Enter *rollup_logstash,opensearch_dashboards_sample_logs* as your *Index Pattern* and `@timestamp`
as the *Time Filter field name*.
+
The notation for a combination index pattern with both raw and rolled up data
is `rollup_logstash,opensearch_dashboards_sample_data_logs`. In this index pattern, `rollup_logstash`
matches the rolled up index pattern and `opensearch_dashboards_sample_data_logs` matches the index
pattern for raw data.

. Open the main menu, click *Dashboard*, then create and add a vertical bar chart.
 
. Choose `rollup_logstash,opensearch_dashboards_sample_data_logs`
