...
behavior of OpenSearchDashboards. For example, you can change the format used to display dates,
...
The `Advanced Settings` {osd} privilege is required to access *Advanced Settings*.
...
indicator in OpenSearchDashboards will be displayed. The buttons to edit settings won't be
visible. For more information on granting access to OpenSearchDashboards, see
<<xpack-security-authorization>>.

[role="screenshot"]
image::images/settings-read-only-badge.png[Example of Advanced Settings Management's read only access indicator in OpenSearchDashboards's header]

[float]
[[opensearch-dashboards-settings-reference]]
=== OpenSearchDashboards settings reference

WARNING: Modifying a setting can affect {osd} performance and cause problems
...
configuration settings. Deleting a custom setting removes it from {osd}
permanently.


[float]
[[opensearch-dashboards-general-settings]]
==== General
...
The timezone that OpenSearchDashboards uses. The default value of `Browser` uses the timezone
...
{ref}/date_nanos.html[OpenSearch date_nanos type].

[[defaultindex]]`defaultIndex`::
The index to access if no index is set. The default is `null`.

[[defaultroute]]`defaultRoute`::
The default route when opening OpenSearchDashboards. Use this setting to route users to a
...
When date histograms use the `auto` interval, OpenSearchDashboards attempts to generate this
number of bars.
...
Fields that exist outside of `_source`. OpenSearchDashboards merges these fields into the
...
The style of navigation menu for OpenSearchDashboards. Choices are Legacy, the legacy style
...
language built specifically for {osd}, and the
...
experimental:[]
OpenSearchDashboards tracks UI state in the URL, which can lead to problems when there is a
...
Set to `true` to enable a dark mode for the {osd} UI. You must refresh the page
...
[float]
[[opensearch-dashboards-accessibility-settings]]
==== Accessibility

[horizontal]
[[accessibility-disableanimations]]`accessibility:disableAnimations`::
Turns off all unnecessary animations in the {osd} UI. Refresh the page to apply
the changes.

[float]
[[opensearch-dashboards-dashboard-settings]]
==== Dashboard

[horizontal]
[[xpackdashboardmode-roles]]`xpackDashboardMode:roles`::
**Deprecated. Use <<opensearch-dashboards-feature-privileges,feature privileges>> instead.**
The roles that belong to <<xpack-dashboard-only-mode, dashboard only mode>>.

[float]
[[opensearch-dashboards-discover-settings]]
==== Discover
...
[float]
[[opensearch-dashboards-ml-settings]]
==== Machine learning
...
[float]
[[opensearch-dashboards-notification-settings]]
==== Notifications
...
[float]
[[opensearch-dashboards-reporting-settings]]
==== Reporting
...
[float]
[[opensearch-dashboards-rollups-settings]]
==== Rollup
...
[float]
[[opensearch-dashboards-search-settings]]
==== Search
...
setting used for `_msearch` requests sent by {osd}. Set to 0 to disable this
config and use the {opensearch} default.
...
[float]
[[opensearch-dashboards-siem-settings]]
==== Security solution
...
A comma-delimited list of {opensearch} indices from which the {security-app} collects
events.
...
[float]
[[opensearch-dashboards-timelion-settings]]
==== Timelion
...
[[timelion-opensearchdefaultindex]]`timelion:opensearch.default_index`::
The default index when using the `.opensearch()` query.

[[timelion-opensearchtimefield]]`timelion:opensearch.timefield`::
The default field containing a timestamp when using the `.opensearch()` query.
...
be selected from an allow-list configured in the `opensearch_dashboards.yml` under
...
[float]
[[opensearch-dashboards-visualization-settings]]
==== Visualization
...
[float]
[[opensearch-dashboards-telemetry-settings]]
==== Usage data
