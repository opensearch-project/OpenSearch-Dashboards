...
Watcher is an {opensearch} feature that you can use to create actions based on
...
NOTE: There are limitations in *Watcher* that affect {osd}. For information, refer to {ref}/watcher-limitations.html[Alerting].

[float]
[[watcher-security]]
=== Watcher security

If the {opensearch} {security-features} are enabled, you must have the
{ref}/security-privileges.html[`manage_watcher` or `monitor_watcher`]
cluster privileges to use Watcher in {osd}.

Alternately, you can have the built-in `opensearch_dashboards_admin` role
...
<<role-management-api, OpenSearchDashboards Role Management API>>. Watches are shared between
...
in {opensearch} for this example to work.
...
Advanced watches are for users who are more familiar with {opensearch} query syntax and
...
identify the watch, and ID refers to the identifier used by {opensearch}.  Refer to
...
* {ref}/watch-cluster-status.html[Watch the status of an {opensearch} cluster]
