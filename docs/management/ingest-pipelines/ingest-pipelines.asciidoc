...
*Ingest Node Pipelines* enables you to create and manage {opensearch}
...
these three items their own field in {opensearch} for fast search and visualization.
...
+
At this point, you’re ready to use the OpenSearch index API to load
the logs data.

. In the OpenSearchDashboards Console, index a document with the pipeline
you created.
+
[source,js]
----------------------------------
