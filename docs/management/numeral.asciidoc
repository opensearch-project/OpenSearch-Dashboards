...
Numeral formatting in {osd} is done through a pattern-based syntax.
...
they are now maintained by {osd}.

Numeral formatting patterns are used in multiple places in {osd}, including:
...
The simplest pattern format is `0`, and the default {osd} pattern is `0,0.[000]`.
...
The display of these patterns is affected by the <<opensearch-dashboards-general-settings, advanced setting>> `format:number:defaultLocale`.
...
The default percentage formatter in {osd} is `0,0.[000]%`, which shows
...
Currency formatting is limited in {osd} due to the limitations of the pattern
