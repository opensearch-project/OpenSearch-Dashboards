...
{osd} requires an index pattern to access the {opensearch} data that you want to explore.
...
* To access the *Index Patterns* view, you must have the {osd} privilege
`Index Pattern Management`.  To create an index pattern, you must have the {opensearch} privilege
`view_index_metadata`. To add the privileges, open the main menu, then click *Stack Management > Roles*.

* If a read-only indicator appears in {osd}, you have insufficient privileges
...
If you collected data using one of the {osd} <<connect-to-opensearch,ingest options>>, uploaded a file, or added sample data,
...
. Start typing in the *Index pattern* field, and {osd} looks for the names of
{opensearch} indices that match your input.
...
. If {osd} detects an index with a timestamp, expand the *Time field* menu,
...
. Click *Create index pattern*.
+
{osd} is now configured to use your {opensearch} data.
...
```ts
rollup_logstash,opensearch_dashboards_sample_data_logs
```
...
If your {opensearch} clusters are configured for {ref}/modules-cross-cluster-search.html[{ccs}],
you can create an index pattern to search across the clusters of your choosing. Use the
same syntax that you'd use in a raw {ccs} request in {opensearch}:

```ts
<cluster-names>:<pattern>
```

For example, to query {ls} indices across two {opensearch} clusters
...
To query across all {opensearch} clusters that have been configured for {ccs},
...
aggregations using that index pattern in {osd} take advantage of {ccs}.
...
which indicates the type of data the field contains in {opensearch},
...
Whenever possible, {osd} uses the same field type for display as
{opensearch}. However, some field types that {opensearch} supports are not available
in {osd}. Using field formatters, you can manually change the field type in {osd} to display your data the way you prefer
to see it, regardless of how it is stored in {opensearch}.

For example, if you store
date values in {opensearch}, you can use a {osd} field formatter to change the display to mm/dd/yyyy format.
{osd} has field formatters for
...
This action also resets the {osd} popularity counters for the fields.
...
This action removes the pattern from the list of saved objects in {osd}.
...
index pattern does not remove any indices or data documents from {opensearch}.
