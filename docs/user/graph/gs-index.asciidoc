...
The {osd} {graph-features} enable you to discover how items in an
OpenSearch index are related. You can explore the connections between
...
interactive graph visualization tool for OpenSearchDashboards. Both work with out of the 
box with existing OpenSearch indices--you don't need to store any 
...
about the documents and terms in your OpenSearch index. A _graph_ is really
...
aren't graph geeks and avoid any confusion with the nodes in an OpenSearch
cluster. 

The graph vertices are simply the terms that you've already indexed. The
connections are derived on the fly using OpenSearch aggregations. To
identify the most _meaningful_ connections, the graph API leverages
OpenSearch relevance scoring. The same data structures and relevance ranking
tools built into OpenSearch to support text searches enable the graph API to
...
But what about performance? The OpenSearch aggregation framework
...
multi-node clusters and scales with your OpenSearch deployment.
