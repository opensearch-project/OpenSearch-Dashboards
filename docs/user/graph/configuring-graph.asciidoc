...
When a user saves a graph workspace in OpenSearchDashboards, it is stored in the `.opensearch-dashboards`
...
`xpack.graph.savePolicy` to `none` in `opensearch_dashboards.yml`:
...
When security is used to grant read only access, the following  indicator in OpenSearchDashboards
is displayed. For more information on granting access to OpenSearchDashboards, see
<<xpack-security-authorization>>.

[role="screenshot"]
image::user/graph/images/graph-read-only-badge.png[Example of Graph's read only access indicator in OpenSearchDashboards's header]
...
`xpack.graph.canEditDrillDownUrls` to `false` in `opensearch_dashboards.yml`:
