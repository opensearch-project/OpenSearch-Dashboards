...
. From the {osd} toolbar, click *Share*, then select *PDF Reports*.
...
. From the {osd} toolbar, click *Share*, then select *CSV Reports*.
...
any problems with using these paths after upgrading {osd}, use
{osd} to regenerate the POST URL for a particular report.
