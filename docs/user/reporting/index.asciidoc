...
You can generate a report that contains a {osd} dashboard, visualization,
...
The {report-features} are automatically enabled in {osd}. It runs a custom build of the Chromium web browser, which
runs on the server in headless mode to load {osd} and capture the rendered {osd} charts as images.

Chromium is an open-source project not related to Elastic, but the Chromium binary for {osd} has been custom-built by Elastic to ensure it
works with minimal setup. However, the {osd} server OS might still require additional dependencies for Chromium. See the
...
the appropriate {osd} privileges to access the objects that you
want to report on and the {opensearch} indices.  See <<secure-reporting, Reporting and security>>
for an example.
...
. In the {osd} toolbar, click *Share*.  If you are working in Canvas,
...
== Layout and sizing
The layout and size of the PDF or PNG image depends on the {osd} app
...
In the following {osd} dashboard, the shareable container is highlighted.
