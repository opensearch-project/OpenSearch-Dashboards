...
== Reporting integration
Integrating a {osd} application with the {report-features} requires a minimum
...
However, these docs will be kept up-to-date to reflect the current implementation of Reporting integration in {osd}.
==============================================

[float]
[[reporting-nav-bar-extensions]]
=== Share menu extensions
X-Pack uses the `share` plugin of the OpenSearchDashboards platform to register actions in the share menu.
