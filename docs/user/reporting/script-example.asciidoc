...
The request method must be `POST` and it must include a `osd-xsrf` header for OpenSearchDashboards
...
<2> Provide user credentials for a user with permission to access OpenSearchDashboards and
{report-features}.
<3> The `osd-xsrf` header is required for all `POST` requests to OpenSearchDashboards. For more information, see <<api-request-headers, API Request
Headers>>.
<4> The POST URL. You can copy and paste the URL for any report from the OpenSearchDashboards UI.
...
<1> The relative path on the OpenSearchDashboards host for downloading the report.
