...
[partintro]
--
**_Tell {osd} where to find your data, then search and filter it for hidden insights and relationships._**
...
search it with the {osd} Query Language, and filter the results.
...
- You must have data in {opensearch}.  This tutorial uses the
<<gs-get-data-into-opensearch-dashboards,ecommerce sample data set>>, but you can use your own data.
- You should have an understanding of {ref}/documents-indices.html[{opensearch} documents and indices].
...
Tell {osd} where to find the data you want to explore, and then specify the time range in which to view that data.

. Open the main menu, and select **Discover**.

. Select the data you want to work with.
+
{osd} uses an <<index-patterns,index pattern>> to tell it where to find
your {opensearch} data.
To view the ecommerce sample data, make sure the index pattern is set to **opensearch_dashboards_sample_data_ecommerce**.
+
[role="screenshot"]
...
build more complex queries, use the <<kuery-query,OpenSearchDashboards Query language>>.
...
+
{osd} creates a visualization best suited for this field.
...
* <<opensearch-dashboards-discover-settings, Configure Discover>> to better meet your needs.
...
include::{osd-repo-dir}/management/index-patterns.asciidoc[]

include::{osd-repo-dir}/discover/set-time-filter.asciidoc[]

include::{osd-repo-dir}/discover/search.asciidoc[]
