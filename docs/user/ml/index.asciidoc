...
{osd} includes a free *{data-viz}* to learn more about your data. In particular,
if your data is stored in {opensearch} and contains a time field, you can use the
...
that data into an {opensearch} index. To change the default file size limit, see
<<opensearch-dashboards-ml-settings,Machine learning advanced settings>>.
...
NOTE: There are limitations in {ml-features} that affect {osd}. For more information, refer to {ml-docs}/ml-limitations.html[Machine learning].
...
false positives. {anomaly-detect-cap} runs in and scales with {opensearch}, and
includes an intuitive UI on the {osd} *Machine Learning* page for creating
...
NOTE: The {osd} {ml-features} use pop-ups. You must configure your web 
...
[role="screenshot"]
image::user/ml/images/outliers.png[{oldetection-cap} results in {osd}]
