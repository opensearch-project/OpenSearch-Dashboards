...
Actions are OpenSearchDashboards services or integrations with third-party systems that run as background tasks on the OpenSearchDashboards server when alert conditions are met. {osd} provides the following types of actions:
...
| Add a message to a OpenSearchDashboards log.
