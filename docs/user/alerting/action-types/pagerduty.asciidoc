...
* Tailor the incident data to match your needs by easily passing the alerting context from OpenSearchDashboards to PagerDuty.

[float]
[[pagerduty-how-it-works]]
===== How it works

{osd} allows you to create alerts to notify you of a significant move
in your dataset.
...
In the `opensearch_dashboards.yml` configuration file, you must add the <<general-alert-action-settings, saved objects encryption setting>>.
...
Transport Security Layer (TLS) enabled for communication <<configuring-tls-osd-opensearch, between OpenSearch and OpenSearchDashboards>>.
...
[[pagerduty-support]]
===== Support
If you need help with this integration, get in touch with the {osd} team by visiting
https://support.elastic.co[support.elastic.co] or by using the *Ask Elastic* option in the {osd} Help menu.
You can also select the {osd} category at https://discuss.elastic.co/[discuss.elastic.co].
...
. Enter an *Integration Name* in the format Elastic-service-name (for example, Elastic-Alerting or OpenSearchDashboards-APM-Alerting)
...
. Create a PagerDuty Connector in OpenSearchDashboards.  You can:
+
...
Source::        An *optional* value indicating the affected system, preferably a hostname or fully qualified domain name. Defaults to the {osd} saved object id of the action.
