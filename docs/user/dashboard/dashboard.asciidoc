...
[role="screenshot"]
image::images/dashboard-read-only-badge.png[Example of Dashboard read only access indicator in OpenSearchDashboards header]
...
the categories that represent the highest and lowest values. OpenSearchDashboards also supports horizontal bar charts.
...
levels of a categorical value. OpenSearchDashboards also supports stacked horizontal bar charts.
...
To create a panel, make sure you have {ref}/getting-started-index.html[data indexed into {opensearch}] and an <<index-patterns,index pattern>>
to retrieve the data from {opensearch}. If you aren’t ready to use your own data, {osd} comes with several pre-built dashboards that you can test out. For more information,
refer to <<add-sample-data, Explore {osd} using sample data>>.
...
{osd} provides you with several editors that help you create panels.
...
*TSVB* is a time series data visualizer that allows you to use the full power of the OpenSearch aggregation framework. To use *TSVB*,
...
With *TSVB*, you can add and display multiple data sets to compare and analyze. {osd} uses many types of <<aggregation-reference,aggregations>> that you can use to build
...
If you want to ignore filters from all of {osd}, select *Yes* for *Ignore global filter*.

[float]
[[vega]]
=== Create custom panels with Vega

Build custom visualizations using *Vega* and *Vega-Lite*, backed by one or more data sources including {opensearch}, Elastic Map Service,
URL, or static data. Use the {osd} extensions to embed *Vega* in your dashboard, and add interactive tools.
...
* <<vega-tutorial-update-opensearch-dashboards-filters-from-vega,Update OpenSearchDashboards filters>>
* <<reference-for-opensearch-dashboards-extensions,Reference for OpenSearchDashboards extensions>>
...
NOTE: In 7.0 and later, *Timelion* app is deprecated. In 8.0 and later, *Timelion* app is removed from {osd}. To prepare for the removal of *Timelion* app, you must migrate *Timelion* app worksheets to a dashboard.
For migration information, refer to <<deprecation-v7.10.0>>.
Only *Timelion* app is deprecated. {osd} continues to support *Timelion*
