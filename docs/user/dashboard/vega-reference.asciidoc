...
[float]
[[reference-for-opensearch-dashboards-extensions]]
=== Reference for {osd} extensions

{osd} has extended Vega and Vega-Lite with extensions that support:

* Automatic sizing
* Default theme to match {osd}
* Writing {opensearch} queries using the time range and filters from dashboards
...
Vega examples, `width` and `height` are not required parameters in {osd}. To set the width
...
The default {osd} settings which are inherited by your visualizations are:
...
{osd} is able to merge your custom `autosize` settings with the defaults. The options `fit-x`
...
{osd} registers a default https://vega.github.io/vega/docs/schemes/[Vega color scheme]
...
[float]
[[vega-queries]]
==== Writing {opensearch} queries in Vega

{osd} extends the Vega https://vega.github.io/vega/docs/data/[data] elements
with support for direct {opensearch} queries specified as `url`.

{osd} is **unable to support dynamically loaded data**,
which would otherwise work in Vega. All data is fetched before it's passed to
the Vega renderer.

To define an {opensearch} query in Vega, set the `url` to an object. {osd} parses
the object looking for special tokens that allow your query to integrate with {osd}.
...
* `{%timefilter%: true}`: Replaced by an {opensearch} range query with upper and lower bounds
...
* `{%autointerval%: true}`: Replaced by the string which contains the automatic {osd} time interval, such as `1h`
...
// is treated as a context-aware OpenSearch query.
url: {
...
----
url: {
  // "type" defaults to "opensearch" otherwise
  type: emsfile
...
{osd} has installed the https://vega.github.io/vega-lite/docs/tooltip.html[Vega tooltip plugin],
so tooltips can be defined in the ways documented there. Beyond that, {osd} also supports
...
Vega can load data from any URL, but this is disabled by default in {osd}. 
To change this, set `vis_type_vega.enableExternalUrls: true` in `opensearch_dashboards.yml`,
then restart {osd}.
...
Vega uses the {ref}/search-search.html[{opensearch} search API] to get documents and aggregation 
results from {opensearch}. To troubleshoot these requests, click *Inspect*, which shows the most recent requests. 
...
Because of the dynamic nature of the data in {opensearch}, it is hard to help you with 
...
* `vega_spec` &mdash; Vega JSON graph specification after some modifications by {osd}. In case
...
{osd} has extended the Vega expression language with these functions:

```js
/**
  * @param {object} query Elastic Query DSL snippet, as used in the query DSL editor
  * @param {string} [index] as defined in OpenSearchDashboards, or default if missing
  */
opensearchDashboardsAddFilter(query, index)

/**
  * @param {object} query Elastic Query DSL snippet, as used in the query DSL editor
  * @param {string} [index] as defined in OpenSearchDashboards, or default if missing
  */
opensearchDashboardsRemoveFilter(query, index)

opensearchDashboardsRemoveAllFilters()
...
/**
  ...
  */
opensearchDashboardsSetTimeFilter(start, end)
```
...
{osd}-specific features like {opensearch} requests and interactive base maps.
...
TIP: When you use the examples in {osd}, you may
