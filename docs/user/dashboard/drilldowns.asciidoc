...
Third-party developers can create drilldowns. To learn how to code drilldowns, refer to {osd-repo}blob/{branch}/x-pack/examples/ui_actions_enhanced_examples[this example plugin].

[float]
[[supported-drilldowns]]
=== Supported drilldowns

{osd} supports two types of drilldowns.
...
To disable URL drilldowns on your {osd} instance, disable the plugin:
...
[source, bash]
----
https://github.com/elastic/opensearch-dashboards/issues?q=is:issue+is:open+{{event.value}}
----
+
The example URL navigates to {osd} issues on Github. `{{event.value}}` is substituted with a value associated with a selected pie slice. In *URL preview*, `{{event.value}}` is substituted with a <<values-in-preview, dummy>> value.
+
[role="screenshot"]
...
. On the page that lists the issues in the {osd} repository, verify the slice value appears in Github.
+
[role="screenshot"]
