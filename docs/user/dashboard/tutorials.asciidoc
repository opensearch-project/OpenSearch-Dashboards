...
. Select the *opensearch_dashboards_sample_data_ecommerce* index pattern.
...
In this tutorial, you will learn about how to edit Vega-Lite in {osd} to create
a stacked area chart from an {opensearch} search query. It will give you a starting point
...
In this tutorial, you will build a stacked area chart from one of the {osd} sample data
sets.
...
When you first open the Vega editor in {osd}, you will see a pre-populated
...
which does not exist in the `opensearch_dashboards_sample_data_ecommerce` data. Find and replace
...
query, it is easiest to use the {osd} Dev Tools in a separate tab from the
...
isolated {opensearch} query into a query with {osd} integration. Looking at the
<<vega-queries, reference for writing {opensearch} queries in Vega>>, you will
...
the results from the {opensearch} query, and `data_0` contains the visually encoded results
...
https://vega.github.io/vega-lite/docs/flatten.html[flatten transformation], available in {osd} 7.9 and later.
If using an older version of OpenSearchDashboards, the flatten transformation is available in Vega
but not Vega-Lite.
...
but the chart is still missing some common features that we expect from a {osd}
...
Because {osd} has enabled the https://vega.github.io/vega-lite/docs/tooltip.html[Vega tooltip plugin],
...
[[vega-tutorial-update-opensearch-dashboards-filters-from-vega]]
=== Update {osd} filters from Vega

experimental[]

In this tutorial you will build an area chart in Vega using an {opensearch} search query,
and add a click handler and drag handler to update {osd} filters.
This tutorial is not a full https://vega.github.io/vega/tutorials/[Vega tutorial],
but will cover the basics of creating Vega visualizations into {osd}.
...
Next, add a valid {opensearch} search query in the `data` block:
  ...
  signals: [
    {
      ...
      on: [{
        ...
        update: '''opensearchDashboardsSetTimeFilter(datum.key, datum.key + 3 * 60 * 60 * 1000)'''
      }]
    }
  ]
```

This event is using the {osd} custom function `opensearchDashboardsSetTimeFilter` to generate a filter that
...
Finally, add a new signal which will update the {osd} time filter when the mouse is released while
dragging:
...
```yaml
    {
      ...
      on: [{
        ...
        update: '''selected[0] !== selected[1] ? opensearchDashboardsSetTimeFilter(
               invert('x',selected[0]),
...
standard visualizations in {osd}, but with the potential to add even more control.
...
[source,text]
----------------------------------
.opensearch(offset=-1h,index=metricbeat-*,
    timefield='@timestamp',
