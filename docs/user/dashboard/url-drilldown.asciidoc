...
[source, bash]
----
https://github.com/elastic/opensearch-dashboards/issues?q={{event.value}}
----
...
|rison
a|Serialize variables in https://github.com/w33ble/rison-node[rison] format. Rison is a common format for {osd} apps for storing state in the URL.
...
* *Global* static variables that don’t change depending on the  place where the URL drilldown is used or which user interaction executed the drilldown. For example: `{{opensearchDashboardsUrl}}`.
...
To work around this, {osd} provides a sample interaction that relies on a trigger.
...
| {osd} base URL. Useful for creating URL drilldowns that navigate within {osd}.
...
| List of {osd} filters applied to a panel. +
Tip: Use in combination with <<helpers, rison>> helper for
internal {osd} navigations with carrying over current filters.
