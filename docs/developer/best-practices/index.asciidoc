...
Consider these best practices, whether developing code directly to the {osd} repo or building your own plugins.
They are intended to support our https://github.com/elastic/engineering/blob/master/opensearch_dashboards_dev_principles.md[{osd} development principals].
...
** If so, have you considered using the streaming {osd-repo}tree/{branch}/src/plugins/bfetch[bfetch service]?

[discrete]
=== Accessibility

Did you know {osd} makes a public statement about our commitment to
creating an accessible product for people with disabilities?
https://www.elastic.co/guide/en/opensearch-dashboards/master/accessibility.html[We do]!
...
[[opensearch-dashboards-localization-best-practices]]
[discrete]
=== Localization

{osd} is translated into other languages. Use our i18n utilities to
ensure your public facing strings will be translated to ensure all
{osd} apps are localized.

* Read and adhere to our
{osd-repo}blob/{branch}/packages/osd-i18n/GUIDELINE.md[i18n
guidelines]
...
{osd-repo}blob/{branch}/STYLEGUIDE.md[styleguide]
(use Typescript!)
* Write all new code on
{osd-repo}blob/{branch}/src/core/README.md[the
platform], and following
{osd-repo}blob/{branch}/src/core/CONVENTIONS.md[conventions].
...
* {osd-repo}tree/{branch}/src/plugins/data/README.md[Data
services]
** {osd-repo}tree/{branch}/src/plugins/data/public/search/README.md[Search
strategies]
*** Use the `opensearchSearchStrategy` to make raw queries to OPENSEARCH that will
...
* {osd-repo}tree/{branch}/src/plugins/embeddable/README.md[Embeddables]
...
* {osd-repo}tree/{branch}/src/plugins/ui_actions/README.md[UiActions]
...
* {osd-repo}tree/{branch}/src/plugins/opensearch_dashboards_utils/docs/state_sync/README.md[state
syncing] and
* {osd-repo}tree/{branch}/src/plugins/opensearch_dashboards_utils/docs/state_containers/README.md[state
container] utilities provided by
* {osd-repo}tree/{branch}/src/plugins/opensearch_dashboards_utils/README.md[opensearch_dashboards_utils]
if you want to sync your application state to the URL?
** {osd-repo}tree/{branch}/src/plugins/opensearch_dashboards_react/README.md[opensearch_dashboards_react]
for react specific helpers

Re-using these services will help create a consistent experience across
{osd} from every solution.
...
{osd-repo}tree/{branch}/src/core/server/saved_objects/migrations/README.md[saved
...
risky changes in minors, _opensearchpecially_ to saved objects.
...
Saved object exported from past {osd} versions should continue to work.
...
<<opensearch-dashboards-navigation, Follow this guide>> to get an idea of available tools and common approaches for handling routing and browser navigation.
