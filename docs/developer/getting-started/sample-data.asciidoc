...
There are a couple ways to easily get data ingested into {opensearch}.
...
prompted to install when running {osd} for the first time. You can also access and install the sample data packages
...
Make sure to execute `node scripts/makelogs` *after* {opensearch} is up and running!
