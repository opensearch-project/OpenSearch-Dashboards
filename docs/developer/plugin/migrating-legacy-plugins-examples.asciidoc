...
legacy APIs to their {osd} Platform equivalents.
...
Legacy Platform, but uses the `@osd/config-schema` package instead of
...
new opensearchDashboards.Plugin({
  config() {
    return Joi.object({
...
{osd-repo}/tree/{branch}/docs/development/core/server/opensearch-dashboards-plugin-core-server.plugininitializercontext.md[PluginInitializerContext]
...
import { CoreSetup, Logger, Plugin, PluginInitializerContext, PluginName } from 'opensearch-dashboards/server';
...
=== Creating a {osd} Platform plugin

For example, if you want to move the legacy `demoplugin` plugin's
configuration to the {osd} Platform, you could create the {osd} Platform plugin with the
same name in `plugins/demoplugin` with the following files:

*plugins/demoplugin/opensearch_dashboards.json*
[source,json5]
----
{
  "id": "demoplugin",
...
[source,typescript]
----
import type { PluginInitializerContext, Plugin, CoreSetup } from 'opensearch-dashboards/server';
...
{osd-repo}/tree/{branch}/docs/development/core/server/opensearch-dashboards-plugin-core-server.httpservicesetup.md[HttpServiceSetup]
interface, which is exposed via the
{osd-repo}/tree/{branch}/docs/development/core/server/opensearch-dashboards-plugin-core-server.coresetup.md[CoreSetup]
...
validated using the `@osd/config-schema` package. If no validation
...
either `server.plugins.*`, `server.*`, or `req.*`. In the {osd} Platform,
all services are available via the `context` argument to the route
handler. The type of this argument is the
{osd-repo}/tree/{branch}/docs/development/core/server/opensearch-dashboards-plugin-core-server.requesthandlercontext.md[RequestHandlerContext].
...
new opensearchDashboards.Plugin({
  init(server) {
    const { callWithRequest } = server.plugins.opensearch.getCluster('data');
  ...
  public setup(core) {
    ...
    router.post(
      ...
      async (context, req, res) => {
        const results = await context.core.opensearch.client.asCurrentUser.search(query);
        return res.ok({
          body: { results }
        });
      }
...
logic that may do some pre-work or validation. In {osd}, these are
often used for license checks.

The {osd} Platform’s HTTP interface does not provide this
...
In the {osd} Platform, the same functionality can be achieved by
...
[source,typescript]
----
// OpenSearchDashboards Platform high-order handler
...
wide range of features. In the {osd} Platform, some of these APIs have
...
In the {osd} Platform, navlinks should not be updated directly. Instead,
...
In {osd}, a `chromeless` application is one where the primary {osd}
...
via the `hidden` property in a {osd} plugin. Chromeless applications
...
In the {osd} Platform, all these registrations are performed
programmatically during your plugin’s `setup` phase, using the core
`savedObjects`’s `registerType` setup API.

The most notable difference is that in the {osd} Platform, the type
...
[source,typescript]
----
import type { SavedObjectsType } from 'opensearch-dashboards/server';
...
The {osd} Platform `registerType` expected input is very close to the legacy format.
...
{osd-repo}/tree/{branch}/docs/development/core/server/opensearch-dashboards-plugin-core-server.savedobjectsnamespacetype.md[SavedObjectsNamespaceType]).
...
The new opensearch client is a thin wrapper around
`@elastic/opensearch`’s `Client` class. Even if the API is quite
close to the legacy client {osd} was previously using, there are some
subtle changes to take into account during migration.

https://www.elastic.co/guide/en/opensearch/client/javascript-api/current/index.html[Official
client documentation]

==== Client API Changes

Refer to the
https://www.elastic.co/guide/en/opensearch/client/javascript-api/current/breaking-changes.html[Breaking
changes list] for more information about the changes between the legacy
and new client.

The most significant changes on the OpenSearchDashboards side for the consumers are the following:
...
===== Response object
Calling any OPENSEARCH endpoint now returns the whole response object instead
...
Note that more information from the OPENSEARCH response is available:
...
const {
  ...
  warnings,    // warnings returned from OPENSEARCH
...
Legacy OpenSearch library until the additional announcements.

[source,typescript]
----
// OpenSearchDashboards provides a few typings for internal purposes
import type { SearchResponse } from 'opensearch-dashboards/server';
type SearchSource = {...};
...
[source,typescript]
----
import { errors } from '@elastic/opensearch';
try {
...
with the OPENSEARCH service internal behavior when scoping as the internal or as
the current user.
...
router.get(
  ...
  async (context, req, res) => {
    const { client } = context.core.opensearch;
...
[source,typescript]
----
const customClient = coreStart.opensearch.legacy.createClient('my-custom-client', customConfig);
...
[source,typescript]
----
const customClient = coreStart.opensearch.createClient('my-custom-client', customConfig);
// do something with the client, such as
