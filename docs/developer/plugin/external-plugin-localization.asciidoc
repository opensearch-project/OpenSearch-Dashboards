...
== Localization for plugins outside the {osd} repo

To introduce localization for your plugin, use our i18n tool to create IDs and default messages. You can then extract these IDs with respective default messages into localization JSON files for {osd} to use when running your plugin.

[discrete]
=== Adding localization to your plugin

You must add a `translations` directory at the root of your plugin. This directory will contain the translation files that {osd} uses.
...
[discrete]
=== Using {osd} i18n tooling
To simplify the localization process, {osd} provides tools for the following functions:

* Verify all translations have translatable strings and extract default messages from templates
* Verify translation files and integrate them into {osd}

To use {osd} i18n tooling, create a `.i18nrc.json` file with the following configs:
...
An example {osd} `.i18nrc.json` is {blob}.i18nrc.json[here].
...
["source","shell"]
-----------
node scripts/i18n_extract --output-dir ./translations --include-config ../opensearch-dashboards-extra/myPlugin/.i18nrc.json
-----------
...
["source","shell"]
-----------
node scripts/i18n_check --fix --include-config ../opensearch-dashboards-extra/myPlugin/.i18nrc.json
-----------
...
{osd} relies on several UI frameworks (ReactJS and AngularJS) and
...
all parts of {osd} (ReactJS, AngularJS and NodeJS).
...
Full details are {osd-repo}tree/master/packages/osd-i18n#vanilla-js[here].
...
Full details are {osd-repo}tree/master/packages/osd-i18n#react[here].
...
You are encouraged to use `i18n.translate()` by statically importing `i18n` from `@osd/i18n` wherever possible in your Angular code. Angular wrappers use the translation `service` with the i18n engine under the hood.
...
Full details are {osd-repo}tree/master/packages/osd-i18n#angularjs[here].
...
* {blob}packages/osd-i18n/README.md[i18n plugin]
* {blob}packages/osd-i18n/GUIDELINE.md[i18n guidelines]
