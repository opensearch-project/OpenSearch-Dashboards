...
`Saved Objects service` allows {osd} plugins to use {opensearch} like a primary
database. Think of it as an Object Document Mapper for {opensearch}. Once a
...
=== Mappings
Each Saved Object type can define it's own {opensearch} field mappings.
...
Will result in the following mappings being applied to the `.opensearch-dashboards` index:
[source,json]
----
{
  "mappings": {
...
specifying `dynamic: false` in any level of your mappings, {opensearch} will
accept and store any other fields even if they are not specified in your mappings.

Since {opensearch} has a default limit of 1000 fields per index, plugins
...
arbitrary amount of fields to be added to the `.opensearch-dashboards` index.
...
Saved Objects support schema changes between OpenSearchDashboards versions, which we call
migrations. Migrations are applied when a OpenSearchDashboards installation is upgraded from
...
specified by the OpenSearchDashboards version number, receive an input document, and must
return the fully migrated document to be persisted to OpenSearch.
...
of a migration must always be the earliest OpenSearchDashboards version in which this
...
will prevent a OpenSearchDashboards upgrade from succeeding and will cause downtime for our
