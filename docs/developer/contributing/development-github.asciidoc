...
forking model] for collaborating on {osd} code. This model assumes that
you have a remote called `upstream` which points to the official {osd}
repo, which we'll refer to in later code snippets.

[discrete]
=== Branching

At Elastic, all products in the stack, including OpenSearchDashboards, are released at the same time with the same version number. Most of these projects have the following branching strategy:
...
error: failed to push some refs to 'https://github.com/YourGitHubHandle/opensearch-dashboards.git'
...
See <<development-pull-request>> for the next steps on getting your code changes merged into {osd}.
