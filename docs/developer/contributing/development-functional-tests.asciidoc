...
We use functional tests to make sure the {osd} UI works as expected. It replaces hours of manual testing by automating user interaction. To have better control over our functional test environment, and to make it more accessible to plugin authors, {osd} uses a tool called the `FunctionalTestRunner`.

[discrete]
=== Running functional tests

The `FunctionalTestRunner` is very bare bones and gets most of its functionality from its config file, located at link:{osd-repo}tree/{branch}/test/functional/config.js[test/functional/config.js]. If you’re writing a plugin outside the {osd} repo, you will have your own config file.
 See <<external-plugin-functional-tests>> for more info.

There are three ways to run the tests depending on your goals:

1. Easiest option:
** Description: Starts up {osd} & {opensearch} servers, followed by running tests. This is much slower when running the tests multiple times because slow startup time for the servers. Recommended for single-runs.
** `node scripts/functional_tests`
*** does everything in a single command, including running {opensearch} and {osd} locally
*** tears down everything after the tests run
*** exit code reports success/failure of the tests

2. Best for development:
** Description: Two commands, run in separate terminals, separate the components that are long-running and slow from those that are ephemeral and fast. Tests can be re-run much faster, and this still runs {opensearch} & {osd} locally.
** `node scripts/functional_tests_server`
*** starts {opensearch} and {osd} servers
*** slow to start
*** can be reused for multiple executions of the tests, thereby saving some time when re-running tests
*** automatically restarts the {osd} server when relevant changes are detected
** `node scripts/functional_test_runner`
*** runs the tests against {osd} & {opensearch} servers that were started by `node scripts/functional_tests_server`
*** exit code reports success or failure of the tests

3. Custom option:
** Description: Runs tests against instances of {opensearch} & {osd} started some other way (like Elastic Cloud, or an instance you are managing in some other way).
** just executes the functional tests
** url, credentials, etc. for {opensearch} and {osd} are specified via environment variables
** Here's an example that runs against an Elastic Cloud instance. Note that you must run the same branch of tests as the version of {osd} you're testing.
+
["source","shell"]
----------
export TEST_OPENSEARCH_DASHBOARDS_URL=https://opensearch-dashboards:password@my-opensearch-dashboards-instance.internal.net:443

export TEST_OPENSEARCH_URL=https://elastic:password@my-opensearch-cluster.internal.net:9200
node scripts/functional_test_runner
----------
...
["source","shell"]
----------
export TEST_OPENSEARCH_DASHBOARDS_PROTOCOL=https
export TEST_OPENSEARCH_DASHBOARDS_HOSTNAME=my-opensearch-dashboards-instance.internal.net
export TEST_OPENSEARCH_DASHBOARDS_PORT=443
export TEST_OPENSEARCH_DASHBOARDS_USER=opensearchDashboards
export TEST_OPENSEARCH_DASHBOARDS_PASS=<password>

export TEST_OPENSEARCH_PROTOCOL=http
export TEST_OPENSEARCH_HOSTNAME=my-opensearch-cluster.internal.net
export TEST_OPENSEARCH_PORT=9200
export TEST_OPENSEARCH_USER=elastic
export TEST_OPENSEARCH_PASS=<password>
node scripts/functional_test_runner
----------

** If you are running x-pack functional tests, start server and runner from the link:{osd-repo}tree/{branch}/x-pack[x-pack] folder:
+
["source", "shell"]
----------
...
** When running against a Cloud deployment, some tests are not applicable. To skip tests that do not apply, use --exclude-tag.  An example shell file can be found at link:{osd-repo}tree/{branch}/test/scripts/jenkins_cloud.sh[test/scripts/jenkins_cloud.sh]
+
["source", "shell"]
----------
...
* `--config test/functional/config.js` starts {opensearch} and {osd} servers with the WebDriver tests configured to run in Chrome.
* `--config test/functional/config.firefox.js` starts {opensearch} and {osd} servers with the WebDriver tests configured to run in Firefox.
* `--config test/api_integration/config.js` starts {opensearch} and {osd} servers with the api integration tests configuration.
* `--config test/accessibility/config.ts` starts {opensearch} and {osd} servers with the WebDriver tests configured to run an accessibility audit using https://www.deque.com/axe/[axe].
...
The tests are written in https://mochajs.org[mocha] using https://github.com/elastic/opensearch-dashboards/tree/master/packages/osd-expect[@osd/expect] for assertions.
...
The `FunctionalTestRunner` automatically transpiles functional tests using babel, so that tests can use the same ECMAScript features that {osd} source code uses. Refer to {osd-repo}tree/6.0/style_guides/js_style_guide.md[style_guides/js_style_guide.md].
...
In order to achieve this a new service was introduced called `createTestUserService` (see link:link:{osd-repo}tree/{branch}/test/common/services/security/test_user.ts[test/common/services/security/test_user.ts]). The purpose of this test user service is to create roles defined in the test config files and setRoles() or restoreDefaults().

An example of how to set the role like how its defined below:

`await security.testUser.setRoles(['opensearch_dashboards_user', 'opensearch_dashboards_date_nanos']);`

Here we are setting the `test_user` to have the `opensearch_dashboards_user` role and also role access to a specific data index (`opensearch_dashboards_date_nanos`).
...
This annotated example file shows the basic structure every test suite uses. It starts by importing https://github.com/elastic/opensearch-dashboards/tree/master/packages/osd-expect[`@osd/expect`] and defining its default export: an anonymous Test Provider. The test provider then destructures the Provider API for the `getService()` and `getPageObjects()` functions. It uses these functions to collect the dependencies of this suite. The rest of the test file will look pretty normal to mocha.js users. `describe()`, `it()`, `before()` and the lot are used to define suites that happen to automate a browser via services and objects of type `PageObject`.
...
export default function ({ getService, getPageObject }) {
  ...
  describe('My Test Suite', () => {
    ...
    // app/page and restores some archives into {opensearch} with opensearchArchiver
    before(async () => {
      await Promise.all([
        // start with an empty .opensearch-dashboards index
        opensearchArchiver.load('empty_opensearch_dashboards'),
        // load some basic log data only if the index doesn't exist
        opensearchArchiver.loadIfNeeded('makelogs')
      ]);
    ...
    // that will tidy up {opensearch} for other test suites
    after(async () => {
      // we unload the empty_opensearch_dashboards archive but not the makelogs
...
[horizontal]
`log`::: An instance of the link:{osd-repo}tree/{branch}/packages/osd-dev-utils/src/tooling_log/tooling_log.ts[`ToolingLog`] that is ready for use
...
**config:**:::
* Source: link:{osd-repo}tree/{branch}/packages/osd-test/src/functional_test_runner/lib/config/config.ts[packages/osd-test/src/functional_test_runner/lib/config/config.ts]
* Schema: link:{osd-repo}tree/{branch}/packages/osd-test/src/functional_test_runner/lib/config/schema.ts[src/functional_test_runner/lib/config/schema.ts]
* Use `config.get(path)` to read any value from the config file

**log:**:::
* Source: link:{osd-repo}tree/{branch}/packages/osd-dev-utils/src/tooling_log/tooling_log.ts[packages/osd-dev-utils/src/tooling_log/tooling_log.ts]
...
**lifecycle:**:::
* Source: link:{osd-repo}tree/{branch}/packages/osd-test/src/functional_test_runner/lib/lifecycle.ts[src/functional_test_runner/lib/lifecycle.ts]
...
The {osd} functional tests define the vast majority of the actual functionality used by tests.

**browser**:::
* Source: link:{osd-repo}tree/{branch}/test/functional/services/common/browser.ts[test/functional/services/common/browser.ts]
...
**testSubjects:**:::
* Source: link:{osd-repo}tree/{branch}/test/functional/services/common/test_subjects.ts[test/functional/services/common/test_subjects.ts]
...
**find:**:::
* Source: {osd-repo}tree/{branch}/test/functional/services/common/find.ts[test/functional/services/common/find.ts]
...
**retry:**:::
* Source: link:{osd-repo}tree/{branch}/test/common/services/retry/retry.ts[test/common/services/retry/retry.ts]
...
* Source: link:{osd-repo}tree/{branch}/test/common/services/opensearch_dashboards_server/opensearch_dashboards_server.ts[test/common/services/opensearch_dashboards_server/opensearch_dashboards_server.ts]
* Helpers for interacting with {osd}'s server
* Commonly used methods:
** `opensearchDashboardsServer.uiSettings.update()`
** `opensearchDashboardsServer.version.get()`
** `opensearchDashboardsServer.status.getOverallState()`

**opensearchArchiver:**:::
* Source: link:{osd-repo}tree/{branch}/test/common/services/opensearcharchiver.ts[test/common/services/opensearcharchiver.ts]
* Load/unload archives created with the `opensearchArchiver`
* Popular methods:
** `opensearchArchiver.load(name)`
** `opensearchArchiver.loadIfNeeded(name)`
** `opensearchArchiver.unload(name)`

Full list of services that are used in functional tests can be found here: link:{osd-repo}tree/{branch}/test/functional/services[test/functional/services]


**Low-level utilities:**:::
* opensearch
** Source: link:{osd-repo}tree/{branch}/test/common/services/opensearch.ts[test/common/services/opensearch.ts]
** {opensearch} client
** Higher level options: `opensearchDashboardsServer.uiSettings` or `opensearchArchiver`
* remote
** Source: link:{osd-repo}tree/{branch}/test/functional/services/remote/remote.ts[test/functional/services/remote/remote.ts]
