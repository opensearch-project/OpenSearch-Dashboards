...
To ensure that your changes will not break other functionality, please run the test suite and build (<<building-opensearch-dashboards>>) before submitting your Pull Request.

[discrete]
=== Running specific {osd} tests
...
|===
|Test runner |Test location |Runner command (working directory is {osd}
root)
...
|Mocha
|`src/**/__tests__/**/*.js` `!src/**/public/__tests__/*.js` `packages/osd-dev-utils/src/**/__tests__/**/*.js` `tasks/**/__tests__/**/*.js`
...
link:{osd-repo}tree/{branch}/x-pack/README.md#testing[X-Pack Testing]
...
Examples: - Run the entire opensearch_service test suite:
`yarn test:jest src/core/server/opensearch/opensearch_service.test.ts`
- Run the jest test case whose description matches
`stops both admin and data clients`:
`yarn test:jest -t 'stops both admin and data clients' src/core/server/opensearch/opensearch_service.test.ts`
...
* Run {osd} with `yarn start --host=computer.local` (substituting
your computer name).
* Now you can run your VM, open the browser, and navigate to
`http://computer.local:5601` to test {osd}.
...
and develop functional tests for {osd} core and plugins.

You can also look into the {osd-repo}tree/{branch}/scripts/README.md[Scripts README.md]
to learn more about using the node scripts we provide for building
{osd}, running integration tests, and starting up {osd} and
{opensearch} while you develop.
