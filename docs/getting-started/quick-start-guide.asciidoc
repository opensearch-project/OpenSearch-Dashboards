...
To quickly get up and running with {osd}, set up on Cloud, then add a sample data set that you can explore and analyze.
...
[float]
=== Required privileges
When security is enabled, you must have `read`, `write`, and `manage` privileges on the `opensearch_dashboards_sample_data_*` indices. 
...
[float]
[[gs-get-data-into-opensearch-dashboards]]
== Add the sample data

Sample data sets come with sample visualizations, dashboards, and more to help you explore {osd} without adding your own data.
...
. To focus in on the documents you want to view, use the <<kuery-query,{osd} Query Language>>. In the *KQL* search field, enter:
+
[source,text]
...
If you are you ready to add your own data, refer to <<connect-to-opensearch,Add data to {osd}>>.
