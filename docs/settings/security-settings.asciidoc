[role="xpack"]
[[security-settings-osd]]
=== Security settings in {osd}
++++
<titleabbrev>Security settings</titleabbrev>
++++

You do not need to configure any additional settings to use the
{security-features} in {osd}. They are enabled by default.
...
| `xpack.security.enabled`
  | By default, {osd} automatically detects whether to enable the
  {security-features} based on the license and whether {opensearch} {security-features}
  are enabled. +
  +
  Do not set this to `false`; it disables the login form, user and role management
  screens, and authorization using <<opensearch-dashboards-privileges>>. To disable
  {security-features} entirely, see
  {ref}/security-settings.html[{opensearch} security settings].
...
You configure authentication settings in the `xpack.security.authc` namespace in `opensearch_dashboards.yml`.
...
The valid settings in the `xpack.security.authc.providers` namespace vary depending on the authentication provider type. For more information, refer to <<opensearch-dashboards-authentication>>.
...
`<provider-type>.<provider-name>.enabled` {opensearchs-icon}
| Determines if the authentication provider should be enabled. By default, {osd} enables the provider as soon as you configure any of its properties.

| `xpack.security.authc.providers.`
`<provider-type>.<provider-name>.order` {opensearchs-icon}
| Order of the provider in the authentication chain and on the Login Selector UI.

| `xpack.security.authc.providers.`
`<provider-type>.<provider-name>.description` {opensearchs-icon}
| Custom description of the provider entry displayed on the Login Selector UI.

| `xpack.security.authc.providers.`
`<provider-type>.<provider-name>.hint` {opensearchs-icon}
| Custom hint for the provider entry displayed on the Login Selector UI.

| `xpack.security.authc.providers.`
`<provider-type>.<provider-name>.icon` {opensearchs-icon}
| Custom icon for the provider entry displayed on the Login Selector UI.

| `xpack.security.authc.providers.`
`<provider-type>.<provider-name>.showInSelector` {opensearchs-icon}
...
| `xpack.security.authc.providers.`
`<provider-type>.<provider-name>.accessAgreement.message` {opensearchs-icon}
| Access agreement text in Markdown format. For more information, refer to <<xpack-security-accopensearchs-agreement>>.
...
|===
| `xpack.security.authc.providers.`
`saml.<provider-name>.realm` {opensearchs-icon}
| SAML realm in {opensearch} that provider should use.

| `xpack.security.authc.providers.`
`saml.<provider-name>.useRelayStateDeepLink` {opensearchs-icon}
| Determines if the provider should treat the `RelayState` parameter as a deep link in {osd} during Identity Provider initiated log in. By default, this setting is set to `false`. The link specified in `RelayState` should be a relative, URL-encoded {osd} URL. For example, the `/app/dashboards#/list` link in `RelayState` parameter would look like this: `RelayState=%2Fapp%2Fdashboards%23%2Flist`.
...
|===
| `xpack.security.authc.providers.`
`oidc.<provider-name>.realm` {opensearchs-icon}
| OpenID Connect realm in {opensearch} that the provider should use.
...
| List of HTTP authentication schemes that {osd} HTTP authentication should support. By default, this setting is set to `['apikey']` to support HTTP authentication with <<api-keys, `ApiKey`>> scheme.
...
[cols="2*<"]
|===
| `xpack.security.loginAssistanceMessage` {opensearchs-icon}
...
|[[xpack-security-encryptionKey]] `xpack.security.encryptionKey`
  | An arbitrary string of 32 characters or more that is used to encrypt session information. Do **not** expose this key to users of {osd}. By
  default, a value is automatically generated in memory. If you use that default
  behavior, all sessions are invalidated when {osd} restarts.
  In addition, high-availability deployments of {osd} will behave unexpectedly
  if this setting isn't the same for all instances of {osd}.
...
|[[xpack-security-secureCookies]] `xpack.security.secureCookies`
  ...
  this to `true` if SSL is configured outside of {osd} (for example, you are
  routing requests through a load balancer or proxy).

| `xpack.security.sameSiteCookies` {opensearchs-icon}
...
|[[xpack-session-lifespan]] `xpack.security.session.lifespan` {opensearchs-icon}
...
| Sets the interval at which {osd} tries to remove expired and invalid sessions from the session index. By default, this value is 1 hour. The minimum value is 10 seconds.
...
[IMPORTANT]
============
In high-availability deployments, make sure you use the same encryption and decryption keys for all instances of {osd}. Although the keys can be specified in clear text in `opensearch_dashboards.yml`, it's recommended to store them securely in the <<secure-settings,{osd} Keystore>>.
============
