...
experimental[] Create a {osd} space.

[[spaces-api-post-request]]
==== Request

`POST <opensearchDashboards host>:<port>/api/spaces/space`

[[spaces-api-post-request-body]]
==== Request body

`id`::
  (Required, string) The space ID that is part of the OpenSearchDashboards URL when inside the space. You are unable to change the ID with the update operation.
