...
experimental[] Create {osd} saved objects.

[[saved-objects-api-create-request]]
==== Request

`POST <opensearchDashboards host>:<port>/api/saved_objects/<type>`

`POST <opensearchDashboards host>:<port>/api/saved_objects/<type>/<id>`

`POST <opensearchDashboards host>:<port>/s/<space_id>/api/saved_objects/<type>`

`POST <opensearchDashboards host>:<port>/s/<space_id>/api/saved_objects/<type>/<id>`
...
arbitrary and ill-formed data into the API that can break {osd}. Make sure
