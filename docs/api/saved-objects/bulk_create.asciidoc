...
experimental[] Create multiple {osd} saved objects.

[[saved-objects-api-bulk-create-request]]
==== Request

`POST <opensearchDashboards host>:<port>/api/saved_objects/_bulk_create`

`POST <opensearchDashboards host>:<port>/s/<space_id>/api/saved_objects/_bulk_create`
