...
experimental[] Update the attributes for existing {osd} saved objects.

[[saved-objects-api-update-request]]
==== Request

`PUT <opensearchDashboards host>:<port>/api/saved_objects/<type>/<id>`

`PUT <opensearchDashboards host>:<port>/s/<space_id>/api/saved_objects/<type>/<id>`
...
+
WARNING: When you update, attributes are not validated, which allows you to pass arbitrary and ill-formed data into the API and break {osd}. Make sure any data that you send to the API is properly formed.
