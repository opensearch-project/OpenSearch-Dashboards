...
experimental[] Retrieve a paginated set of {osd} saved objects by various conditions.

[[saved-objects-api-find-request]]
==== Request

`GET <opensearchDashboards host>:<port>/api/saved_objects/_find`

`GET <opensearchDashboards host>:<port>/s/<space_id>/api/saved_objects/_find`
...
`search`::
  (Optional, string) An OpenSearch {ref}/query-dsl-simple-query-string-query.html[simple_query_string] query that filters the objects in the response.
...
NOTE: As objects change in {osd}, the results on each page of the response also
