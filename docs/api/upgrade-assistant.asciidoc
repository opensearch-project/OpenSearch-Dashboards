...
Check the upgrade status of your {opensearch} cluster and reindex indices that were created in the previous major version. The assistant helps you prepare for the next major version of {opensearch}.
...
* <<cancel-reindex, Cancel reindex API>> to cancel reindexes that are waiting for the {opensearch} reindex task to complete
