...
`GET <opensearchDashboards host>:<port>/api/upgrade_assistant/reindex/myIndex`
...
<4> Task ID of the reindex task in OpenSearch. Only present if reindexing has started.
<5> Percentage of how far the reindexing task in OpenSearch has progressed, in decimal from from 0 to 1.
...
`3`::
  Paused
+
NOTE: If the {osd} node that started the reindex is shutdown or restarted, the reindex goes into a paused state after some time.
...
`0`::
  The reindex operation has been created in OpenSearchDashboards.
...
`40`::
  The reindex task in OpenSearch has started.

`50`::
  The reindex task in OpenSearch has completed.
